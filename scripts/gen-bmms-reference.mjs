...
// Every keyword needs a line, in both languages.
//
// The editor's hover explains the word under the pointer from `sched.kw.<word>`. A keyword
// added to the compiler arrives here automatically (that is the point of extracting them), and
// without an entry the hover shows the bare word — which reads as "this feature does not work"
// rather than "somebody has not written this sentence yet".
//
// Bailed on rather than warned about, for the same reason as the action check above: a page
// that silently falls back looks finished, and nobody re-reads a generated file.
for (const lang of ['en', 'fr']) {
  const missing = keywords.filter((k) => !DICT[lang]['sched.kw.' + k]);
  if (missing.length) {
    bail(`${lang}.json has no sched.kw.* line for: ${missing.join(', ')} — the editor hover would show the bare word`);
  }
}
