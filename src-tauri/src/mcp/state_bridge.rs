...
/// Generate a repo.json for a server
pub fn generate_repo(name: &str, mods_to_include: Vec<String>) -> anyhow::Result<String> {
    let data = read_app_data()?;
    let mut repo_mods = Vec::new();
    
    for id in mods_to_include {
        if let Some(m) = data.mods.iter().find(|m| m.id == id || m.name == id) {
    ...
    let repo_json = serde_json::json!({
        "name": name,
        "description": format!("Repository generated by BMM AI on {}", chrono::Local::now().format("%Y-%m-%d")),
        "mods": repo_mods,
        "version": "1.0.0"
    });
    
    let repo_path = std::env::current_dir()?.join("repo.json");
    std::fs::write(&repo_path, serde_json::to_string_pretty(&repo_json)?)?;
    
    Ok(format!("Repository manifest 'repo.json' generated at {:?}", repo_path))
}
