...
const HOSTILE = {
  ...
  perms: { command: true, script: true, deeplink: true, stopProcess: true, delete: true, resources: true },
...
describe('sanitiseImportedTask', () => {
  ...
  test('every outside-BMM capability is taken away', () => {
    ...
    assert.deepEqual(task.perms, { command: false, script: false, deeplink: false, stopProcess: false, delete: false, resources: false });
  ...
  test('it REPORTS what was asked for, so the person is not left wondering', () => {
    ...
    assert.deepEqual([...strippedPerms].sort(), ['command', 'deeplink', 'delete', 'resources', 'script', 'stopProcess']);
