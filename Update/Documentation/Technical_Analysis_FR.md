...
| **Surface d'Outils** | 73 outils atomiques exposés via le binaire `mcp-server`, plus 66 sous-commandes CLI sur le même binaire, couvrant toute la surface de commande de BMM. |
...
| Aspect | Détail |
| :--- | :--- |
| **Routes** | ~89 endpoints via `path!("api" / ...)` : `health`, `status`, `mods` (+ `active`/`enable`/`disable`/`{id}`), `profiles` (+ `activate`/`{id}`), `plugins` (+ `compare`/`apply`), `modpacks` (+ `create`/`enable`/`disable`/`import`/`{id}`), `repo` (`info`/`connect`/`list`/`sync`/`gen`/`host`), `data` (`export`/`import`), `modlists` (`export`/`import`), `creator-id`, `check-update`, `restart`. |
