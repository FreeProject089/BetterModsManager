...
| `/api/catalogs` | yes | `{ type, url, follow, password?, key? }` | GET lists followed catalogues, POST follows or unfollows one. `key` names which identity key signs the request — an id or a name, shown in Settings → Identity & API. A reference not on the ring is reported rather than skipped. |
