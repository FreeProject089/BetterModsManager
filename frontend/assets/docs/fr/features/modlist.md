...
[modpack](doc-page:modpacks), il embarque les liens de téléchargement : la personne qui le reçoit
...
différent n'ont **pas** le même jeu — voir [les conflits](doc-page:library#conflicts).
...
porté par la liste. Coche **profil auto** et il construit un [profil](doc-page:profiles) dédié
...
| **Arbre de fichiers** | La disposition des fichiers du mod (chemins et tailles), et des **hachages** par fichier optionnels. C'est ce qui alimente la vérification et ce que BMM compare pour trouver les [conflits](doc-page:library#conflicts). |
...
!!! note "« Règles de conflit » = l'ordre"
    ...
    celui activé en dernier gagne (voir [conflits](doc-page:library#conflicts)). Reproduire « à
