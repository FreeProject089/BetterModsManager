...
export function promptRepoPassword(): Promise<string | null> {
...
function initSyncKeyAndSshFields(): void {
    ...
    const sel = document.getElementById('repo-sync-ssh-target') as HTMLSelectElement | null;
    ...
    const refresh = async () => {
        ...
        if (!names.length) {
            const o = document.createElement('option');
            o.value = ''; o.textContent = t('repo.sync.useSshNotSet');
            sel.appendChild(o);
        }
    };
    ...
    document.getElementById('btn-sync-ssh-config')?.addEventListener('click', () => {
        ...
        (document.getElementById('repo-ssh-host') as HTMLInputElement | null)?.focus();
    });
    void refresh();
}
...
async function bcwebProfileForAuthor(authorId: string | null | undefined): Promise<string | null> {
    if (!authorId) return null;
    try {
        const base = ((getLinks() as any)?.bettercommunity || 'https://bettercommunity.ch/').replace(/\/+$/, '');
        ...
        const hit = (JSON.parse(txt)?.users || []).find((u: any) => u?.matchedById && !u?.private);
...
async function decorateAuthorLink(el: HTMLElement | null, authorId: string | null | undefined): Promise<void> {
    if (!el) return;
...
document.addEventListener('click', (e) => {
    const el = (e.target as HTMLElement)?.closest('.repo-author-linked') as HTMLElement | null;
...
async function isBcLinked(): Promise<boolean> {
    try {
        ...
        const txt = await invoke('bc_api_get', { url: `${base}/api/link/status?creatorId=${encodeURIComponent(cid as string)}` }) as string;
        const d = JSON.parse(txt);
        try { localStorage.setItem('bc_linked', d?.linked ? '1' : '0'); } catch {}
        return !!d?.linked;
    } catch {
        try { return localStorage.getItem('bc_linked') === '1'; } catch { return false; }
    }
}
...
function _openRepoVerifyDetail(repo: any, isVerified: boolean, reason?: string): void {
    const modal = document.getElementById('modal-repo-verify-detail');
    if (!modal) return;

    const totalMods  = repo.profiles ? repo.profiles.reduce((n: number, p: any) => n + (p.mods?.length || 0), 0) : 0;
    ...
    void decorateAuthorLink(document.getElementById('repo-vd-author'), (repo as any).author_id);
...
export function initRepoSync(elements) {
        ...
        btnFetchInfo.addEventListener('click', async () => {
            ...
            try {
                ...
                try {
                    ...
                    let host = '';
                    try { host = new URL(url.replace(/\/repo\.json$/i, '')).host; } catch {}
                ...
                const expectedSig = (window as any).__bmmRepoExpectedSig?.[normUrl];
                ...
                void decorateAuthorLink(syncAuthorDisplay as HTMLElement, (repo as any).author_id);
                ...
                (syncBadge as any)._repoRef = repo;
            ...
            try {
                ...
                if (window.__TAURI__) {
                    ...
                    unlisten = await listen('bmm://repo-sync-progress', (event) => {
                        ...
                        if (current_file) syncDetails.textContent = current_file;
                    });
                }
    ...
    if (syncBadge) {
        syncBadge.addEventListener('click', () => {
            const repo = (syncBadge as any)._repoRef;
            if (!repo) return;
