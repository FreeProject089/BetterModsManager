...
export function initRepo() {
    ...
    const initRepoUpdate = () => {
        ...
        const refreshPublishBtn = async () => {
            ...
            // than no button.
            const dir = (document.getElementById('repo-update-path') as HTMLInputElement | null)?.value?.trim() || '';
        ...
        void import('./repo-extras.js').then((m) => {
            m.mountExtrasButton(
                document.getElementById('repo-update-ssh-mount')?.parentElement || null,
                () => (document.getElementById('repo-update-path') as HTMLInputElement | null)?.value?.trim() || '',
            );
        });
    ...
    if (btnExtras) {
        ...
        void import('./repo-extras.js').then((m) => {
            m.wireExtrasButton(btnExtras, () => (elements.inputExportPath as HTMLInputElement | null)?.value?.trim() || '');
        });
    }
