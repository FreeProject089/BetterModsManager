...
export function initRepoServer(elements) {
    const {
        ...
        upnpBadgeStatus,
        publicHintBox,
        repoCreatorIdContainer,
        repoCreatorIdValue,
        tunnelSection,
        tunnelUrlInput,
        btnCopyTunnelUrl,
        inputServerPort,
        inputServerUploadLimit,
        serverTools,
        btnGenMiniServer,
        btnPickMiniRepo,
        btnPickMiniFolder,
...
const ICON_DONE = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg>';
    ...
    async function subscribeServerEvents() {
        ...
        _unlistenDlStarted = await listen('bmm://server-download-started', (event) => {
            ...
            toast(
                `${t('repo.notifDownloadStarted')} — ${protocol} [${ip}]\n${id} · ${shortFile} (${formatBytes(total_size)})`,
                'info',
                4000,
                ICON_DOWN,
            );
            ...
            if (isServerRunning) {
                ...
                try {
                    ...
                    if (tunnelSection) {
                        ...
                        if (tunnelSection) tunnelSection.style.display = 'none';
                    }
    ...
    if (btnGenMiniServer) {
        btnGenMiniServer.addEventListener('click', async () => {
            ...
            if (!jsonPath) return toast(t('repo.errNoOutDir'), 'warning');
