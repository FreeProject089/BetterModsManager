...
function renderCreate(container: HTMLElement) {
    ...
    const bundledChip = (rel: string, removed: boolean, kind: 'script' | 'folder' | 'automation') => {
...
function getDeepLinkDefs(): DeepLinkDef[] {
    return [
        ...
        },
        {
            scheme: 'view/open',
            params: [
                { name: 'id', required: true, desc: 'L\'écran : library, profiles, modpacks, modlist, mapper, repo, apps, plugins, community, docs, settings, credits.' },
            ],
            desc: 'Ouvrir un écran de BMM',
            about: 'Bascule l\'app sur un écran, comme un clic dans la barre latérale. Un id qui ne correspond à aucun écran ne fait rien et le dit à la console — il n\'y a pas d\'écran « inconnu » à afficher.',
            example: 'bmm://view/open?id=repo',
        },
        {
            scheme: 'schedule/enable',
            params: [
                { name: 'id', required: true, desc: 'L\'id de la tâche.' },
                { name: 'on', required: false, desc: '1 pour armer, 0 pour désarmer. Défaut : 1.' },
            ],
            desc: 'Armer ou désarmer une tâche planifiée',
            about: 'Demande d\'abord, et la question nomme la tâche plutôt que son id. Les ids sont frappés comme <code>sched-&lt;timestamp&gt;</code> — prévisibles d\'une façon qu\'un id aléatoire n\'est pas — donc désarmer la tâche de quelqu\'un ne doit pas pouvoir se faire en devinant quand elle a été créée. Désarmer n\'arrête pas une exécution en cours : ça arrête la suivante.',
            example: 'bmm://schedule/enable?id=sched-1712345678901&on=0',
        },
        {
            scheme: 'catalog/follow',
            params: [
                { name: 'type', required: true, desc: 'app, plugin, theme, preset, modpack, repo, tutorial ou list.' },
                { name: 'url',  required: true,  desc: 'L\'adresse du catalogue.' },
                { name: 'password', required: false, desc: 'Si le catalogue est protégé. Retenu pour cette session uniquement, jamais écrit sur le disque.' },
            ],
            desc: 'Suivre un catalogue',
            about: 'Ajoute une source à la liste des catalogues suivis du type indiqué. <code>bmm://catalog/unfollow</code> fait l\'inverse avec les mêmes paramètres. Un type qui n\'existe pas est nommé plutôt qu\'ignoré : « rien ne s\'est passé » pour une faute de frappe est ce sur quoi on perd un après-midi.',
            example: 'bmm://catalog/follow?type=plugin&url=https://exemple.org/catalog.json',
        },
        {
            scheme: 'repo/publish-ssh',
            params: [
                { name: 'dir', required: true, desc: 'Le dossier de dépôt exporté à téléverser.' },
            ],
            desc: 'Publier un dépôt par SSH — téléverse immédiatement',
            about: 'Envoie vers le serveur SSH déjà enregistré dans Dépôt Serveur, et répond quand le transfert est fini — il n\'ouvre aucun écran. Ne porte ni hôte, ni utilisateur, ni chemin de clé : un lien capable de les nommer pourrait faire lire à BMM une clé privée de son choix et expédier un dépôt vers une machine de son choix. <code>bmm://repo/fetch-ssh</code> est l\'inverse, et demande confirmation — publier écrase des fichiers sur un serveur que vous avez configuré, récupérer écrase les vôtres.',
            example: 'bmm://repo/publish-ssh?dir=D:/repos/dcs',
        },
        {
            scheme: 'hook',
            params: [
                { name: 'name', required: true, desc: 'Le nom du hook, tel qu\'une tâche l\'attend.' },
            ],
            desc: 'Faire sonner un hook',
            about: 'Réveille toute tâche planifiée qui attend ce nom. C\'est la façon dont quelque chose d\'extérieur à BMM — un launcher, un script, un bouton sur une page — déclenche une automatisation sans avoir à connaître son id.',
