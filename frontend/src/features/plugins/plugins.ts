...
import {
    ...
    looksLikeIndex, importIndexForType, describeKinds,
    catalogLooksLike, readSources, STORE_KEY } from '../catalogs/catalog-index.js';
...
let _editAutomations: string[] = []; // .bmmpa files already shipped, carried over when editing
let _removedAutomations: string[] = []; // staged for removal — undo until save
let _editBundles: string[] = [];        // .bmmbundle files already shipped
let _removedBundles: string[] = [];     // staged for removal — undo until save
let _renderPcAutomations: (() => void) | null = null;
let _renderPcBundles: (() => void) | null = null;
let _allProfiles = [];
let _apiToken = '';
let _exePath = '';
...
function renderTab(tabId: string) {
    ...
    switch (tabId) {
        ...
        case 'scripts':
            ...
            setTimeout(() => { if (_tab === 'scripts' && document.getElementById('plug-tab-content') === container) renderScripts(container); }, 0);
            break;
...
function buildPluginCard(plugin: any, source: 'installed' | 'catalog') {
    ...
    const assetsBtn = card.querySelector('.plug-btn-assets') as HTMLElement | null;
...
function renderCatalogGrid(plugins: any[], query = '') {
    const grid = document.getElementById('plug-catalog-grid');
    if (!grid) return;
    ...
    if (!plugins.length) {
        ...
        if (q) {
            ...
            grid.querySelector('#plug-empty-clear')?.addEventListener('click', () => {
                const inp = document.getElementById('plug-catalog-search') as HTMLInputElement | null;
                if (inp) inp.value = '';
                renderCatalogGrid(_catalog?.plugins || []);
                inp?.focus();
            });
        ...
        grid.querySelector('#plug-empty-sources')?.addEventListener('click', () => (document.getElementById('plug-toggle-sources') as HTMLElement | null)?.click());
        grid.querySelector('#plug-empty-import')?.addEventListener('click', () => (document.getElementById('plug-import-file-cat') as HTMLElement | null)?.click());
        grid.querySelector('#plug-empty-refresh')?.addEventListener('click', () => (document.getElementById('plug-refresh-catalog') as HTMLElement | null)?.click());
        return;
    }
    grid.innerHTML = '';
...
function renderUqtForm(ep: EndpointDef, formEl: HTMLElement): void {
     ...
     */
    const markInvalid = (el: HTMLElement | null, bad: boolean): void => {
        if (el) el.classList.toggle('plug-uqt-bad', bad);
    };
    const validate = (): boolean => {
        let ok = true;
        formEl.querySelectorAll<HTMLElement>('[data-freq="1"]').forEach(el => {
        ...
        formEl.querySelectorAll<HTMLTextAreaElement>('.plug-uqt-json').forEach(el => {
            const raw = el.value.trim();
            if (!raw) return;
            try { JSON.parse(raw); markInvalid(el, false); }
            catch { markInvalid(el, true); ok = false; }
        });
        return ok;
    };
    ...
    formEl.querySelectorAll<HTMLTextAreaElement>('.plug-uqt-json').forEach(el => {
        const msg = formEl.querySelector<HTMLElement>(`.plug-uqt-jsonmsg[data-for="${el.id}"]`);
        el.addEventListener('input', () => {
            const raw = el.value.trim();
            if (!raw) { markInvalid(el, false); if (msg) msg.textContent = ''; return; }
            try { JSON.parse(raw); markInvalid(el, false); if (msg) { msg.textContent = ''; msg.className = 'plug-uqt-jsonmsg'; } }
            catch (e: any) { markInvalid(el, true); if (msg) { msg.textContent = String(e?.message || 'invalid JSON'); msg.className = 'plug-uqt-jsonmsg is-bad'; } }
        });
    });
    formEl.querySelectorAll<HTMLElement>('[data-freq="1"]').forEach(el => {
    ...
    formEl.querySelector('#uqt-reset')?.addEventListener('click', () => {
        formEl.querySelectorAll<HTMLInputElement>('input, textarea, select').forEach(el => {
            if (el.type === 'checkbox') el.checked = false; else el.value = '';
            markInvalid(el, false);
        });
        formEl.querySelectorAll<HTMLElement>('.plug-uqt-jsonmsg').forEach(m => { m.textContent = ''; m.className = 'plug-uqt-jsonmsg'; });
    });
    ...
    formEl.querySelector('#uqt-run')?.addEventListener('click', () => {
        ...
        if (!validate()) {
            ...
            formEl.querySelector<HTMLElement>('.plug-uqt-bad')?.focus();
            return;
        }
...
async function openSmartQuickTest(m: string, p: string, rawBody: string) {
    ...
    overlay.querySelector('#plug-qt-s-copy')?.addEventListener('click', async () => {
        ...
        if (p === '/api/repo/gen') {
            ...
            const compression = (overlay.querySelector('#plug-qt-s-compression')  as HTMLSelectElement)?.value || 'deflate';
    ...
    overlay.querySelector('#plug-qt-s-run')?.addEventListener('click', () => {
        ...
        // ── Repo API ──────────────────────────────────────────────────────────
            ...
            const md = (overlay.querySelector('#plug-qt-s-manifest-dir') as HTMLInputElement)?.value?.trim() || '';
            if (!md) return;
            overlay.remove();
...
function setShipCount(id: string, n: number): void {
...
function renderCreate(container: HTMLElement) {
    ...
    const bundledChip = (rel: string, removed: boolean, kind: 'script' | 'folder' | 'automation' | 'bundle') => {
    ...
    const renderAutomationsList = () => {
        ...
        list.querySelectorAll('.plug-auto-rm').forEach(b => b.addEventListener('click', () => {
            automationPaths.splice(parseInt((b as HTMLElement).dataset.i!, 10), 1);
            renderAutomationsList();
        }));
        list.querySelectorAll('.plug-bundled-rm').forEach(b => b.addEventListener('click', () => {
            const rel = (b as HTMLElement).dataset.rel!;
            if (!_removedAutomations.includes(rel)) _removedAutomations.push(rel);
            renderAutomationsList();
        }));
        list.querySelectorAll('.plug-bundled-undo').forEach(b => b.addEventListener('click', () => {
            const rel = (b as HTMLElement).dataset.rel!;
    ...
    container.querySelector('#pc-import-automations')?.addEventListener('click', async () => {
        ...
        if (f) { automationPaths.push(f as string); renderAutomationsList(); }
    });
    ...
    const renderBundlesList = () => {
        ...
        list.querySelectorAll('.plug-bundle-rm').forEach(b => b.addEventListener('click', () => {
            bundlePaths.splice(parseInt((b as HTMLElement).dataset.i!, 10), 1);
            renderBundlesList();
        }));
        list.querySelectorAll('.plug-bundled-rm').forEach(b => b.addEventListener('click', () => {
            const rel = (b as HTMLElement).dataset.rel!;
            if (!_removedBundles.includes(rel)) _removedBundles.push(rel);
            renderBundlesList();
        }));
        list.querySelectorAll('.plug-bundled-undo').forEach(b => b.addEventListener('click', () => {
            const rel = (b as HTMLElement).dataset.rel!;
    ...
    container.querySelector('#pc-import-bundles')?.addEventListener('click', async () => {
        ...
        if (f) { bundlePaths.push(f as string); renderBundlesList(); }
    });
    ...
    return [
        ...
        },
        {
            ...
            params: [
                ...
                { name: 'key', required: false, desc: t('plugins.dl.catKey') },
                { name: 'passphrase', required: false, desc: t('plugins.dl.catPassphrase') },
            ],
        ...
        },
        {
            ...
            desc: t('plugins.dl.catUnfollowDesc'),
            about: t('plugins.dl.catUnfollowAbout'),
            example: 'bmm://catalog/unfollow?type=plugin&url=https://exemple.org/catalog.json',
        },
        {
            scheme: 'catalog/import',
            params: [
                { name: 'url',  required: true,  desc: t('plugins.dl.catUrl') },
                { name: 'type', required: false, desc: t('plugins.dl.catImpType') },
                { name: 'password', required: false, desc: t('plugins.dl.catPw') },
                { name: 'key', required: false, desc: t('plugins.dl.catKey') },
                { name: 'passphrase', required: false, desc: t('plugins.dl.catPassphrase') },
            ],
            desc: t('plugins.dl.catImpDesc'),
            about: t('plugins.dl.catImpAbout'),
            example: 'bmm://catalog/import?url=https://exemple.org/index.json',
        },
        {
            scheme: 'catalog/publish',
            params: [
                { name: 'kind', required: false, desc: t('plugins.dl.catPubKind') },
                { name: 'dir',  required: true,  desc: t('plugins.dl.catPubDir') },
                { name: 'name', required: false, desc: t('plugins.dl.catPubName') },
                { name: 'base', required: false, desc: t('plugins.dl.catPubBase') },
            ],
            desc: t('plugins.dl.catPubDesc'),
            about: t('plugins.dl.catPubAbout'),
            example: 'bmm://catalog/publish?kind=theme&dir=C:/out&name=My%20themes',
        },
        {
            scheme: 'catalog/entry',
            params: [
                { name: 'mode', required: false, desc: t('plugins.dl.catEntryMode') },
                { name: 'type', required: true,  desc: t('plugins.dl.catType') },
                { name: 'id',   required: false, desc: t('plugins.dl.catEntryId') },
                { name: 'fields', required: false, desc: t('plugins.dl.catEntryFields') },
            ],
            desc: t('plugins.dl.catEntryDesc'),
            about: t('plugins.dl.catEntryAbout'),
            example: 'bmm://catalog/entry?mode=add&type=theme&fields=%7B%22id%22%3A%22neon%22%7D',
        },
        {
            scheme: 'catalog/delete',
            params: [
                { name: 'type', required: true, desc: t('plugins.dl.catType') },
            ],
            desc: t('plugins.dl.catDelDesc'),
            about: t('plugins.dl.catDelAbout'),
            example: 'bmm://catalog/delete?type=theme',
        },
        {
            scheme: 'repo/fetch-ssh',
            params: [
                { name: 'dir', required: true, desc: t('plugins.dl.fetchDir') },
            ],
            desc: t('plugins.dl.fetchDesc'),
            about: t('plugins.dl.fetchAbout'),
            example: 'bmm://repo/fetch-ssh?dir=C:/repos/mine',
        },
        {
        ...
        },
        {
            ...
            params: [
                { name: 'name', required: true, desc: t('plugins.dl.hookName') },
                            { name: 'data', required: false, desc: t('plugins.dl.hookData') },
            ],
        ...
        },
        {
            ...
            params: [
                ...
                { name: 'mb',      required: false, desc: t('plugins.dl.benchmark_open.p.mb') },
                            { name: 'mode', required: false, desc: t('plugins.dl.bmMode') },
                { name: 'sources', required: false, desc: t('plugins.dl.bmSources') },
                { name: 'folders', required: false, desc: t('plugins.dl.bmFolders') },
                { name: 'profiles', required: false, desc: t('plugins.dl.bmProfiles') },
            ],
        ...
        // ── Automatisation / exécution ───────────────────────────────────────
        {
            ...
            params: [{ name: 'id', required: true, desc: t('plugins.dl.schedule_run.p.id') },
                { name: 'on', required: false, desc: t('plugins.dl.schedRunOn') },
            ],
        ...
        },
        {
            ...
            params: [                { name: 'path', required: false, desc: t('plugins.dl.replayPath') },
            ],
    ...
    return [
        ...
        },
        {
            ...
            fields: [
                ...
                { name: 'passphrase', type: 'string', required: false, desc: t('plugins.epF.cfPassphrase') },
            ],
            responseStatuses: [
...
const SCRIPT_EXT: Record<string, string> = { bmms: 'bmmscript' };
...
async function handleSaveScriptZip() {
    ...
    // Build README.md
    const extMap: Record<string,string> = { bat:'cmd', ps1:'powershell', vbs:'cscript', py:'python', lua:'lua', js:'node', rb:'ruby', php:'php', go:'go run', java:'javac + java', cs:'dotnet run', rs:'cargo run', bmms:'BMM itself' };
...
function _actionsNeedApi(actions: Array<{ action_type: string }>, useDeeplink: boolean): boolean {
    return actions.some(a => {
        const hasApi = apiBodyFor(a) !== null;
...
async function handleApply(pluginId: string) {
    ...
    if (applyMode === 'automation') { await installPluginAutomations(pluginId, true); return; }
 ...
 */
async function installPluginBundles(pluginId: string): Promise<void> {
    let files: { name: string; path: string }[];
    try {
        files = await invoke('plugin_bundles', { pluginId }) as typeof files;
    } catch (e) { toast(`${t('common.error')}: ${e}`, 'error', 8000); return; }
    if (!files.length) { toast(t('plugins.bundle.none'), 'info', 6000); return; }
    ...
    const found: { name: string; path: string; kind: string }[] = [];
    const failed: string[] = [];
    for (const f of files) {
        try {
            const res: any = await invoke('catalog_bundle_open', { path: f.path });
            const doc = JSON.parse(String(res?.catalog || ''));
            ...
            if (!kind || (kind !== 'app' && !STORE_KEY[kind])) { failed.push(f.name); continue; }
            found.push({ name: f.name, path: f.path, kind });
        } catch { failed.push(f.name); }
    }
    if (failed.length) toast(`${t('plugins.bundle.someBad')} ${failed.join(', ')}`, 'warning', 9000);
    if (!found.length) return;

    const lines = found.map((f) => `${f.name} — ${t('plugins.catKind.' + f.kind) || f.kind}`).join('\n');
    const ok = await showConfirm(
        t('plugins.bundle.followTitle'),
        `${t('plugins.bundle.followBody')}\n\n${lines}\n\n${t('plugins.bundle.followSafety')}`,
        false,
    );
    if (!ok) return;
    ...
    for (const f of found) {
        ...
        try {
            ...
            if (list.includes(src)) { already += 1; continue; }
            list.push(src);
            writeSources(key, list);
            recordHistory({ action: 'add', type: f.kind, url: src, via: `plugin:${pluginId}` });
            added += 1;
        } catch (e) { toast(`${f.name}: ${e}`, 'error', 8000); }
    }
    if (added) toast(`${t('plugins.bundle.followed').replace('{n}', String(added))}`, 'success', 9000);
...
async function installPluginAutomations(pluginId: string, runNow: boolean): Promise<void> {
    ...
    try {
        files = await invoke('plugin_automations', { pluginId }) as typeof files;
    } catch (e) { toast(`${t('common.error')}: ${e}`, 'error', 8000); return; }
    if (!files.length) { toast(t('plugins.auto.none'), 'info', 6000); return; }
    ...
    for (const f of files) {
        let doc: any;
        try { doc = JSON.parse(f.text); } catch { failed.push(f.name); continue; }
        ...
        if (doc && doc.includes) {
            try { sched.writeBlocks({ ...sched.readBlocks(), ...doc.includes }); } catch { /* keep going */ }
        }
        if (!tasks.length) { failed.push(f.name); continue; }
    ...
    const ok = await showConfirm(
        t('plugins.auto.runTitle'),
        `${t('plugins.auto.runBody')}\n\n${names}\n\n${t('plugins.auto.runSafety')}`,
        false,
    );
    if (!ok) {
    ...
    for (const { id, name } of imported) {
        if (!id) continue;
        try { await sched.runTaskById(id); }
        catch (e) { toast(`${name}: ${e}`, 'error', 8000); }
    }
}
