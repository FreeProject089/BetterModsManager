 ...
 */
function describeOrigin(mod: any): string {
    const host = (u: string) => { try { return new URL(u).host; } catch { return String(u).slice(0, 48); } };
    if (mod.source_repo) {
        return (mod.unverified ? t('detail.originRepoUnverified') : t('detail.originRepo'))
            .replace('{h}', host(mod.source_repo));
    }
    if (mod.update_url) return t('detail.originUpdate').replace('{h}', host(mod.update_url));
...
export async function renderModDetail(modId) {
  ...
  wireSourceAccess('mdl', (m, k) => toast(t(m), k === 'warning' ? 'warning' : 'success'),
    ...
    () => (panel.querySelector('.detail-link-url') as HTMLInputElement | null)?.value?.trim() || '');
