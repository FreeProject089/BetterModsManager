...
export async function importListAsking(path: string): Promise<any> {
    try {
        ...
        if (!String(e).includes('mm.errLocked')) throw e;
...
export function initModlist() {
    ...
    const hashToggle = document.getElementById('mm-include-hashes') as HTMLInputElement | null;
    const hashesBadge = document.getElementById('mm-hashes-badge');
    hashToggle?.addEventListener('change', () => {
        if (hashesBadge) hashesBadge.style.opacity = hashToggle.checked ? '1' : '0.3';
    });
