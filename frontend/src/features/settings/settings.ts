...
export async function initSettings() {
    ...
    const scanIntervalEl = document.getElementById('scan-interval-sec') as HTMLInputElement | null;
    if (scanIntervalEl) {
        try { scanIntervalEl.value = String(parseInt(localStorage.getItem('bmm_scan_interval_sec') || '0', 10) || 0); } catch { scanIntervalEl.value = '0'; }
        scanIntervalEl.addEventListener('change', () => {
            ...
            try { localStorage.setItem('bmm_scan_interval_sec', String(v)); } catch {}
        });
    }
