...
async function send(linked: boolean, cfg: Awaited<ReturnType<typeof fetchFeedbackConfig>>): Promise<void> {
    ...
    try {
        ...
        say(t('fbm.sending'));
        const r = await submitFeedback({ kind: _kind, title: title || undefined, body, email: email || undefined, discord: discord || undefined, meta: { steps: steps.length, crashZips: _zips.length, screenshots: _shots.length }, attachments });
        try {
