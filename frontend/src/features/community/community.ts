...
type Post = {
  ...
  reactionTypes?: string[]; reactionsEnabled?: boolean; status?: string;
};
...
function fmtDate(d?: string) { try { return d ? new Date(d).toLocaleDateString(locale(), { year: 'numeric', month: 'short', day: 'numeric' }) : ''; } catch { return ''; } }
...
function projMono(p: Post, sizeCls = ''): string {
  const key = projOf(p);
  const logo = PROJ_LOGO[key];
  if (logo) return `<img class="community-cover-logo${sizeCls}" src="${logo}" alt="" />`;
...
function coverBox(p: Post, cls: string, art: number, sizeCls: string): string {
  if (p.cover) return `<div class="${cls} community-cover-img" style="background-image:url('${escAttr(absUrl(p.cover))}')"></div>`;
...
function card(p: Post, art: number): string {
...
function leadPost(p: Post, latest: boolean): string {
  const { title, excerpt } = pick(p);
  return `
...
function archiveRow(p: Post, art: number): string {
  const { title, excerpt } = pick(p);
  let day = '', weekday = '';
  try {
    const d = p.publishedAt ? new Date(p.publishedAt) : null;
    if (d && !isNaN(d.getTime())) { day = String(d.getDate()); weekday = d.toLocaleDateString(locale(), { weekday: 'short' }); }
...
function byMonth(list: Post[]): { key: string; label: string; posts: Post[] }[] {
  const groups: { key: string; label: string; posts: Post[] }[] = [];
  for (const p of list) {
    let d: Date | null = null;
    try { const x = p.publishedAt ? new Date(p.publishedAt) : null; if (x && !isNaN(x.getTime())) d = x; } catch { /* undated */ }
    const key = d ? `${d.getFullYear()}-${d.getMonth()}` : 'none';
    let g = groups[groups.length - 1];
    if (!g || g.key !== key) {
      let label = t('community.undated');
      if (d) { try { label = d.toLocaleDateString(locale(), { month: 'long', year: 'numeric' }); } catch { label = String(d.getFullYear()); } }
      g = { key, label, posts: [] };
      groups.push(g);
    }
    g.posts.push(p);
...
function edition(list: Post[]): string {
  ...
  const art = new Map<Post, number>(list.map((p, i) => [p, i] as [Post, number]));
...
function tagLabel(label: string | null): string { return label ?? t('community.filter.community'); }
function tagCount(n: number): string { return n === 1 ? t('community.filter.countOne') : t('community.filter.count', { n: String(n) }); }
...
function tagMark(tag: { projectKey: string; label: string | null; logo: string }): string {
  if (tag.logo) return `<img class="community-cover-logo community-mark--xs" src="${escAttr(absUrl(tag.logo))}" alt="" />`;
...
function tagSelect(tags: ReturnType<typeof blogTags> | null): string {
  const total = (tags || []).reduce((n, tg) => n + tg.count, 0);
  const opt = (value: string, label: string, icon: string, count: number | null) =>
    `<option value="${escAttr(value)}"${value === _filter ? ' selected' : ''} data-icon="${escAttr(icon)}"${count == null ? '' : ` data-desc="${escAttr(tagCount(count))}"`}>${escHtml(label)}</option>`;
...
function wire(): void {
  if (!_view) return;
  const tagSel = _view.querySelector('#community-tag') as HTMLSelectElement | null;
  if (tagSel) tagSel.addEventListener('change', () => {
    _filter = tagSel.value || ALL_TAGS; render();
    ...
    setTimeout(() => (_view?.querySelector('.bmm-csel-trigger.community-tag-select') as HTMLElement | null)?.focus({ preventScroll: true }), 0);
  });
