...
class DebugUI {
    ...
    toggle(force) {
        ...
        if (willOpen && !appState.get('debugMode') && force !== true) return;
    ...
    attachListeners() {
        ...
        this._get('rust-mem-snap')?.addEventListener('click', async () => {
            const btn = this._get('rust-mem-snap') as HTMLButtonElement | null;
            ...
            try {
                ...
                if (btn) { btn.disabled = false; btn.textContent = window.t?.('dev.btn.memSnap') || 'MEMORY'; }
            }
        });
        ...
        debugHub.subscribe(this._hubSub = event => {
            if (event.type === 'crash') {
                ...
                if (det) det.textContent = event.data?.msg || 'Unknown internal error';
                return;
            }
    ...
    switchTab(tabId) {
        ...
        if (tabId === 'timeline' && !wasTimeline) this.rebuildTimeline();
    ...
    destroy() {
        this.isOpen = false;
        try { this.toggleInspector(false); } catch { /* ignore */ }
