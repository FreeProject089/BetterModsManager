...
function inline(s: string): string {
  ...
  s = s.replace(/:icon\[([^\]]+)\](?:\{[^}]*\})?/g, (_m, raw) => {
    ...
    if (ph) { const w = ph[1] || 'regular'; return keep(`<span class="doc-icon doc-icon-mask" data-ph="${w}/${ph[2]}${w === 'regular' ? '' : `-${w}`}"></span>`); }
    // Isometric (G5): `iso:server`, bundled under assets/icons/iso, drawn in its own colours.
    const iso = isoIconUrl(n);
    if (iso) return keep(`<img class="doc-icon doc-icon-iso" src="${iso}" alt="" loading="lazy">`);
  ...
  s = s.replace(/:audio\[([^\]]*)\](?:\{([^}]*)\})?/g, (_m, txt, rawAttrs) => {
    const a = leafAttrs(rawAttrs); const raw = a.src || a.href || ''; const src = safeDocUrl(raw) ? raw : '';
    if (!src) return esc(txt);
  ...
  s = s.replace(/\[\[([^\]|#\n]*)(?:#([^\]|\n]+))?(?:\|([^\]\n]+))?\]\]/g, (_m, page, hash, text) =>
    keep(`<span class="doc-ref" title="${escRaw(String(page || hash || '').trim())}">${esc(String(text || page || hash || '').trim())}</span>`));
...
function renderDirective(dir: { name: string; label: string; attrs: Record<string, string> }, body: string[]): string {
  ...
  if (name === 'timeline') return `<div class="doc-timeline">${label || attrs.title ? `<div class="doc-timeline-title">${esc(label || attrs.title)}</div>` : ''}${inner()}</div>`;
  ...
  if (name === 'compare') {
    ...
    for (const side of ['before', 'after']) {
      if (attrs[side]) body = body.replace(new RegExp(`(<div class="doc-compare-label")( data-md-${side})?(>)<span data-md-side><\\/span>`), `$1$3${esc(attrs[side])}`);
    }
  ...
  if (name === 'stats') return `<div class="doc-stats">${inner()}</div>`;
  ...
  if (name === 'changelog') return `<div class="doc-changelog">${label || attrs.title ? `<div class="doc-changelog-title">${esc(label || attrs.title)}</div>` : ''}${inner()}</div>`;
  ...
  if (name === 'faq') return `<div class="doc-faq">${label || attrs.title ? `<div class="doc-faq-title">${esc(label || attrs.title)}</div>` : ''}${inner()}</div>`;
  ...
  if (name === 'audio') {
    const raw = attrs.src || attrs.href || ''; const src = safeDocUrl(raw) ? raw : '';
...
function promoteLeaves(src: string): string {
  return src.replace(/^[ \t]*:(file|ref|card)\[([^\]]*)\](\{[^}]*\})?[ \t]*$/gm,
    ...
    .replace(/^[ \t]*::?(spotify|youtube|yt|audio|include|embed-md|openapi|swagger|live|image|img)(\[[^\]]*\])?(\{[^}]*\})?[ \t]*$/gm,
      (_m, name, label, at) => `:::${name}${label || ''}${at || ''}\n:::`);
}
