...
flowchart TD
    ...
    subgraph EXECUTION ["{{docs.diagram.cluster.EXECUTION}}"]
        PREVIEW["<div class='node-content'><i class='icon-verify'></i> {{docs.diagram.mapper.node.PREVIEW}}</div>"]
        MOVE["<div class='node-content'><i class='icon-refresh'></i> {{docs.diagram.mapper.node.MOVE}}</div>"]
        SUCCESS["<div class='node-content'><i class='icon-verify'></i> {{docs.diagram.mapper.node.SUCCESS}}</div>"]
    end
    ...
    REGISTRY -- "{{docs.diagram.mapper.edge.REG_PREV}}" --> PREVIEW
