 ...
 */
function pdfUrl(dark = isDarkTheme()): string {
...
export function notifyNewArticles(): void {
  const all = allArticleIds();
  const known = new Set(lsArr(DOCS_KNOWN_KEY));
  if (known.size === 0) { lsPut(DOCS_KNOWN_KEY, all); return; } // first ever run — adopt, don't shout
  const fresh = all.filter((id) => !known.has(id));
  if (fresh.length) {
    for (const id of fresh) unreadSet.add(id);
    ...
    try { toast(msg, 'info', 6000, 'book'); } catch { /* toast unavailable this early */ }
  }
  lsPut(DOCS_KNOWN_KEY, all);
}

/** Clear an article's "New" pill once opened (whichever path opened it). */
function markArticleSeen(artId: string): void {
  if (unreadSet.delete(artId)) lsPut(DOCS_UNREAD_KEY, [...unreadSet]);
}
    ...
    articles: [
      {
        ...
        body: {
          en: '<p>Hosting turns a profile into a <b>source of truth</b> others subscribe to — a squadron, a community, or just keeping your own machines identical.</p>'
            ...
            + '<h4>Three ways to produce it</h4><p>They are alternatives, in <b>Server Repo → Host</b>: <b>Full export</b> copies every mod into a folder you upload; <b>Manifest only</b> writes just <code>repo.json</code> for folders already on this machine — one, several, or a set of profiles — copying nothing; <b>Update from the server</b> reads what your server holds — an HTTP directory listing, or SFTP on an SSH machine, where no index has to be published at all — and writes the manifest without pulling the repo back. A layout template (<code>{id}</code>/<code>{path}</code>) lets an existing server keep its own folder shape. Re-running any of them keeps the repo’s identity, re-signs it, and reports what was added, changed and removed.</p><h4>Serving it</h4><p>Open <b>Server Repo</b> and pick what to share. Then either run BMM’s <b>built-in mini-server</b>, or generate a small standalone server (Node, or a <code>.bat</code>/<code>.sh</code> script) to run on a dedicated machine. In the Generate card, <b>Compress as .zip</b> only packs the repo; the standalone server is a separate <b>optional</b> tick inside it (the API’s <code>zipOutput</code> and <code>generateServer</code> flags are independent for the same reason). A <b>Compression</b> picker beside the zip boxes chooses the method for the archive and the per-mod zips — Deflate (default, read by every unzipper), Zstandard (much faster, about as small), Bzip2 (smaller, slower) or Stored (none) — and the same <code>compression</code> value is accepted by the <code>repo.genNow</code> scheduler action, <code>/api/repo/gen</code> and <code>/api/repo/gen-now</code>, the script generator, the CLI and the MCP tool. The Generate and Manifest-only cards are split into numbered sections you open one at a time, every card on the Host tab folds from the chevron in its header, and the <b>multi-repo hub</b> (one Node server + dashboard for all your repos) is its own card in the Serve step. Serve it over HTTP — <b>HTTPS is strongly recommended</b>. Hand out the resulting link.</p>'
      ...
      },
      {
        ...
        body: {
          en: '<p>Catalogs are feeds of ready-to-install content — mods, apps, themes and plugins — that BMM reads from a URL. The <b>App Catalog</b> screen browses them; installing is one click (BMM handles the download and, for apps, the setup).</p><ul><li><b>Official</b> catalogs are curated; you can also add a <b>community</b> catalog by URL.</li><li>Install buttons are plain <code>bmm://</code> deeplinks, so a catalog can live on any website — or in the BetterCommunity hub.</li><li>Publish your own through <b>BetterCommunity</b>. Note it’s a separate hosted service from a self-hosted server repo.</li></ul><h4>Or publish one yourself, with no host at all</h4><p>Every kind of catalogue BMM reads, it can now write: automations, plugins, themes, tutorials, modpacks and shared mod lists. Each entry is either <b>packed</b> into the catalogue or <b>linked</b> to an address you give — chosen per entry, so one catalogue can carry the three small things and point at the 90&nbsp;MB one somebody already hosts.</p><p>The result is a folder you can upload anywhere static, or — ticking <i>Publish it as ONE file</i> — a single <code>.bmmbundle</code> holding the catalogue and everything it packs. That one file is followable on its own: nothing to host, no address to keep alive. It is a zip with its own extension, so anyone can look inside without BMM.</p>',
          fr: '<p>Les catalogues sont des flux de contenu prêt à installer — mods, applis, thèmes et plugins — que BMM lit depuis une URL. L’écran <b>App Catalog</b> les parcourt ; l’installation se fait en un clic (BMM gère le téléchargement et, pour les applis, l’installation).</p><ul><li>Les catalogues <b>officiels</b> sont sélectionnés ; vous pouvez aussi ajouter un catalogue <b>communautaire</b> par URL.</li><li>Les boutons d’installation sont de simples deeplinks <code>bmm://</code>, un catalogue peut donc vivre sur n’importe quel site — ou dans le hub BetterCommunity.</li><li>Publiez les vôtres via <b>BetterCommunity</b>. C’est un service hébergé, distinct d’un dépôt serveur auto-hébergé.</li></ul><h4>Ou publiez-en un vous-même, sans aucun hébergeur</h4><p>Chaque type de catalogue que BMM lit, il sait désormais l’écrire : automatisations, plugins, thèmes, tutoriels, modpacks et listes de mods partagées. Chaque entrée est soit <b>emballée</b> dans le catalogue, soit <b>liée</b> à une adresse que vous donnez — au choix par entrée, un même catalogue peut donc porter les trois petites choses et pointer vers celle de 90&nbsp;Mo que quelqu’un héberge déjà.</p><p>Le résultat est un dossier à téléverser sur n’importe quel hébergement statique, ou — en cochant <i>Publier en UN seul fichier</i> — un unique <code>.bmmbundle</code> contenant le catalogue et tout ce qu’il emballe. Ce seul fichier se suit tel quel : rien à héberger, aucune adresse à maintenir en vie. C’est un zip avec sa propre extension, n’importe qui peut regarder dedans sans BMM.</p>',
        },
      },
      {
    ...
    articles: [
      {
        ...
        body: {
          en: '<p>BMM ships <b>13 built-in themes</b> — dark ones (Default, Graphite, Pure Black, Indigo, Neon, Arctic, Editor, Cobalt…) and four light ones (Daylight, Newsprint, Clay, Blossom). Switch in <b>Settings → Themes</b>, or make your own.</p>'
          ...
          fr: '<p>BMM livre <b>13 thèmes intégrés</b> — des sombres (Default, Graphite, Pure Black, Indigo, Neon, Arctic, Editor, Cobalt…) et quatre clairs (Daylight, Newsprint, Clay, Blossom). Changez dans <b>Réglages → Thèmes</b>, ou créez le vôtre.</p>'
...
La référence complète champ par champ, avec chaque clé optionnelle et un tableau de diagnostic, est sur le site de documentation.`,
      ...
      {
        id: 'content-ids', docsPath: 'reference/content-ids',
        title: { en: 'Ids: local and content', fr: 'Ids\u00a0: local et de contenu' },
        summary: { en: 'Two ids per thing, and why the difference matters when you compare setups.', fr: 'Deux ids par chose, et pourquoi la diff\u00e9rence compte quand on compare deux installations.' },
        keywords: 'id content id copy compare share bmmc1 identifiant contenu copier comparer partager',
        body: {
          en: '<p>Everything BMM holds has a <b>local id</b> — the name this machine gave it. Nine kinds also have a <b>content id</b>, which names <i>what the thing is</i>.</p><ul><li>The <b>local id</b> goes in a <code>bmm://</code> link, an API call, a step in an automation. It is different on every machine.</li><li>The <b>content id</b> starts <code>bmmc1:</code> and is the same wherever the content is the same. It is the one to quote when asking somebody whether they have this.</li></ul><p>Both are on the card, beside the name — the plain button copies the local id, the one with a dot copies the content id. Modpacks, plugins, automations, profiles, launch packs, themes, apps, mod lists and repos all have them.</p><p>The rule is the same for every kind: <b>hash what the thing IS, never what it is called here.</b> A modpack is its members, so the order they were added does not matter; an automation is its steps, so their order is the whole thing. A profile is its game and what is switched on, not its name or any of its paths. A theme is its colours, so renaming a downloaded one does not hide that you both have it.</p><p>A content id is a hash of content, so anyone with the same content can compute it. It proves two things match; it is not a credential and it is not a secret.</p>',
          fr: '<p>Tout ce que BMM contient a un <b>id local</b> — le nom que cette machine lui a donné. Neuf types ont aussi un <b>id de contenu</b>, qui nomme <i>ce que la chose est</i>.</p><ul><li>L\'<b>id local</b> va dans un lien <code>bmm://</code>, un appel API, une étape d\'automatisation. Il diffère sur chaque machine.</li><li>L\'<b>id de contenu</b> commence par <code>bmmc1:</code> et est identique partout où le contenu l\'est. C\'est celui à citer pour demander à quelqu\'un s\'il a la même chose.</li></ul><p>Les deux sont sur la carte, à côté du nom — le bouton simple copie l\'id local, celui avec un point copie l\'id de contenu. Modpacks, plugins, automatisations, profils, launch packs, thèmes, applications, listes de mods et repos en ont tous.</p><p>La règle est la même pour chaque type : <b>hacher ce que la chose EST, jamais le nom qu\'elle porte ici.</b> Un modpack est ses membres, donc l\'ordre d\'ajout ne compte pas ; une automatisation est ses étapes, donc leur ordre est tout. Un profil est son jeu et ce qui est activé, pas son nom ni aucun de ses chemins. Un thème est ses couleurs, donc renommer celui qu\'on a téléchargé ne cache pas que vous l\'avez tous les deux.</p><p>Un id de contenu est un hachage de contenu : quiconque a le même contenu peut le calculer. Il prouve que deux choses correspondent ; ce n\'est ni un secret ni un identifiant d\'authentification.</p>',
        },
      },
      {
      ...
      },
      {
        ...
        title: { en: 'B.MD — better.markdown', fr: 'B.MD — better.markdown' },
        ...
        body: {
          en: '<p>Anywhere BMM renders markdown — a plugin’s documentation, a custom page, a community article — you get ordinary markdown <b>plus</b> a set of blocks. Each one opens with <code>:::name</code> and closes with a bare <code>:::</code>.</p>'
            ...
            + '<li><b>Badges and icons</b> — <code>:badge[New]{color=#0a7}</code>, <code>:icon[rocket]</code>, and Phosphor icons: <code>:icon[ph:rocket]</code>, <code>:icon[ph-bold:rocket]</code> (thin, light, regular, bold, fill, duotone).</li>'
            + '<li><b>Timeline</b> — <code>:::timeline[Title]</code> around <code>:::event[Title]{date="2026-09-01" state=done}</code> blocks; a state is <i>done</i>, <i>now</i> or <i>next</i>.</li>'
            + '<li><b>Before / after</b> — <code>:::compare{before="v1" after="v2"}</code> holding <code>:::before</code> and <code>:::after</code>.</li>'
            + '<li><b>Stats</b> — <code>:::stats</code> around <code>:::stat[Label]{value="12 400" delta="+8%" icon=download}</code> tiles; the delta’s sign picks its colour.</li>'
            + '<li><b>Quote</b> — <code>:::quote[Author]{role="…" avatar=…}</code>, a pull quote with a name under it.</li>'
            + '<li><b>Hero</b> — <code>:::hero[Title]{subtitle="…" image=… align=center}</code>, a banner.</li>'
            + '<li><b>Changelog</b> — <code>:::changelog</code> around <code>:::version[1.4.0]{date="…" label=latest}</code> entries; the <code>[NEW]</code> / <code>[FIXED]</code> chips work inside.</li>'
            + '<li><b>Spoiler</b> — <code>:::spoiler[Reveal]</code>, hidden until clicked. <b>FAQ</b> — <code>:::faq[Title]</code> around <code>:::q[Question]</code> entries.</li>'
            + '<li><b>Checklist</b> — <code>:::checklist[Title]</code> around a <code>- [x]</code> task list; the header counts the ticks.</li>'
            + '<li><b>Grid</b> — <code>:::grid{cols=3}</code>, a fixed number of columns. <b>Meter</b> — <code>:meter[60]{label=Done}</code>, an inline progress bar.</li>'
          ...
          fr: '<p>Partout où BMM affiche du markdown — la documentation d’un plugin, une page personnalisée, un article de la communauté — vous avez le markdown ordinaire <b>plus</b> un jeu de blocs. Chacun s’ouvre par <code>:::nom</code> et se ferme par un <code>:::</code> seul.</p>'
            ...
            + '<li><b>Badges et icônes</b> — <code>:badge[Nouveau]{color=#0a7}</code>, <code>:icon[rocket]</code>, et les icônes Phosphor : <code>:icon[ph:rocket]</code>, <code>:icon[ph-bold:rocket]</code> (thin, light, regular, bold, fill, duotone).</li>'
            + '<li><b>Chronologie</b> — <code>:::timeline[Titre]</code> autour de blocs <code>:::event[Titre]{date="2026-09-01" state=done}</code> ; l’état est <i>done</i>, <i>now</i> ou <i>next</i>.</li>'
            + '<li><b>Avant / après</b> — <code>:::compare{before="v1" after="v2"}</code> contenant <code>:::before</code> et <code>:::after</code>.</li>'
            + '<li><b>Stats</b> — <code>:::stats</code> autour de tuiles <code>:::stat[Libellé]{value="12 400" delta="+8%" icon=download}</code> ; le signe du delta choisit sa couleur.</li>'
            + '<li><b>Citation</b> — <code>:::quote[Auteur]{role="…" avatar=…}</code>, une citation signée.</li>'
            + '<li><b>Hero</b> — <code>:::hero[Titre]{subtitle="…" image=… align=center}</code>, une bannière.</li>'
            + '<li><b>Changelog</b> — <code>:::changelog</code> autour d’entrées <code>:::version[1.4.0]{date="…" label=latest}</code> ; les puces <code>[NEW]</code> / <code>[FIXED]</code> marchent dedans.</li>'
            + '<li><b>Spoiler</b> — <code>:::spoiler[Révéler]</code>, caché jusqu’au clic. <b>FAQ</b> — <code>:::faq[Titre]</code> autour d’entrées <code>:::q[Question]</code>.</li>'
            + '<li><b>Checklist</b> — <code>:::checklist[Titre]</code> autour d’une liste de tâches <code>- [x]</code> ; l’en-tête compte les cases cochées.</li>'
            + '<li><b>Grille</b> — <code>:::grid{cols=3}</code>, un nombre fixe de colonnes. <b>Jauge</b> — <code>:meter[60]{label=Fait}</code>, une barre de progression en ligne.</li>'
      ...
      },
      {
        ...
        summary: { en: 'A real automation builder — triggers, conditions, loops and 107 actions.', fr: 'Un vrai constructeur d’automatisations — déclencheurs, conditions, boucles et 107 actions.' },
...
- **CORS now covers failures too.** The rejection handler runs after the CORS filter, so a recovered response carried no CORS headers of its own — and rather than leave the app unable to read its own \`401\`s it stamped \`access-control-allow-origin: *\` on every failure. That was a token oracle: a wrong token gave a READABLE \`401\` and a right one gave a success the filter refused to expose, two outcomes any web page could tell apart. Both paths read one list now, and an unrecognised site reads neither.

Six entries that used to be here are gone because the code changed: the permission gates (every route listed now declares a scope, and the three \`/api/apps/permissions*\` routes take the **admin** token), the duplicated \`serverVersion\` on \`POST /api/repo/gen\`, the wrong description of \`POST /api/repo/host\`, the missing \`bmm://\` badge on \`DELETE /api/plugins/:id\`, \`bmm://telemetry/settings\`, and the CORS leak above. Four are now held by a check instead of a note — a note goes stale in silence.
...
- **Le CORS couvre aussi les échecs.** Le gestionnaire de rejet tourne après le filtre CORS : une réponse récupérée ne portait aucun en-tête CORS — et plutôt que de laisser l’app incapable de lire ses propres \`401\`, il posait \`access-control-allow-origin: *\` sur chaque échec. C’était un oracle à token : un mauvais token donnait un \`401\` LISIBLE, un bon un succès que le filtre refusait d’exposer — deux résultats qu’une page web sait distinguer. Les deux chemins lisent une seule liste maintenant, et un site inconnu ne lit ni l’un ni l’autre.

Six entrées qui figuraient ici ont disparu parce que le code a changé : les barrières de permission (chaque route listée déclare désormais une portée, et les trois routes \`/api/apps/permissions*\` exigent le token **admin**), le \`serverVersion\` en double sur \`POST /api/repo/gen\`, la description fausse de \`POST /api/repo/host\`, le badge \`bmm://\` manquant sur \`DELETE /api/plugins/:id\`, \`bmm://telemetry/settings\`, et la fuite CORS ci-dessus. Quatre sont maintenant tenues par un check et non par une note — une note se périme en silence.
...
Automatise-le depuis le [Planificateur](doc:scheduler) : benchmarke un disque, applique une limite, ou vérifie l’espace libre et branche sur le résultat.`,
  ...
  },
  {
    ...
    articles: [
      ...
      devArticle('feedback-reporting', { en: 'Feedback & crash reports', fr: 'Retours & rapports de plantage' }, { en: 'Where Settings → Feedback & bug reports sends things.', fr: 'Où partent les envois de Réglages → Retours & rapports de bug.' }, 'feedback bug crash report betahub bettercommunity', {
        en: '<p>The in-app reporter packages your description plus context — logs, DxDiag, a crash zip, screenshots — and posts it to the <b>BetterCommunity feedback centre</b> (<code>feedback_endpoint</code> in links.json; empty = the older BetaHub pipeline). With a linked account the report opens a thread under <b>Messages &amp; reports</b> in your dashboard and a staff reply shows up in BMM\'s notification centre; anonymous reports with an e-mail are answered by mail.</p><p>BetterCommunity being unreachable never blocks you: the report is kept locally and sent on the next start. BMM also throttles itself (5 per 10 min, 20 per day) and the server applies its own per-IP / per-account limits and per-project caps (size, crash sampling, versions).</p>',
        fr: '<p>Le rapporteur intégré empaquette votre description plus le contexte — logs, DxDiag, zip de plantage, captures — et l\'envoie au <b>centre de retours BetterCommunity</b> (<code>feedback_endpoint</code> dans links.json ; vide = l\'ancien pipeline BetaHub). Avec un compte lié, le rapport ouvre un fil dans <b>Messages &amp; signalements</b> de votre tableau de bord et une réponse du staff apparaît dans le centre de notifications de BMM ; les rapports anonymes avec e-mail sont répondus par mail.</p><p>Un BetterCommunity injoignable ne bloque jamais : le rapport est gardé localement et envoyé au prochain démarrage. BMM se limite aussi lui-même (5 par 10 min, 20 par jour) et le serveur applique ses propres limites par IP / par compte et ses plafonds par projet (taille, échantillonnage des plantages, versions).</p>',
      }),
    ],
  },
];
...
function devArticle(diagramId: string, title: L, summary: L, keywords: string, body: L, docsPath = '#', id = diagramId): Article {
  return {
...
// ── state + host ─────────────────────────────────────────────────────────────────
type View = 'hub' | 'cat' | 'art' | 'diagrams' | 'search' | 'pages' | 'page';
type SearchMode = 'classic' | 'semantic';
interface Route { view: View; part: Part; catId?: string; artId?: string; q?: string; mode: SearchMode; page?: string; }
...
function crumbs(): string {
  ...
  if (route.view === 'pages' || route.view === 'page') {
    ...
    if (route.view === 'pages') { parts.push(sep, `<span class="dh-crumb on">${all}</span>`); return parts.join(''); }
    const meta = (_manifest || []).find((p) => p.path === route.page);
    parts.push(sep, `<button class="dh-crumb" data-view2="pages">${all}</button>`,
      sep, `<span class="dh-crumb on">${meta ? tr(meta.title) : route.page}</span>`);
    return parts.join('');
  }
...
function articleView(cat: Category, a: Article): string {
  ...
  return `
    <article class="dh-article${a.wide ? ' dh-wide' : ''}">
      ...
      <div class="dh-content">${renderDocMarkdown(tr(a.body), { trusted: true })}</div>
...
function paint() {
  ...
  if (route.view === 'art') {
    ...
    if (path && (_manifest || []).some((p) => p.path === path)) {
      const btn = host?.querySelector('[data-fullpage]') as HTMLElement | null;
  ...
  input.addEventListener('input', () => {
    ...
    deb = window.setTimeout(() => {
      ...
      if (q.length >= 2) {
        ...
        if (!_manifest) void loadManifest().then(() => { if (route.view === 'search') paint(); });
      } else go({ view: 'hub', q: '' });
    }, 120);
  });
}
...
function onClick(e: Event) {
  ...
  if (v2) {
    ...
    if (want === 'pages') { void openPages(); return; }        // needs the manifest first
  ...
  if (artBtn) { const f = findArticle(artBtn.getAttribute('data-art') || ''); if (f) { markArticleSeen(f.art.id); go({ view: 'art', part: f.cat.part, catId: f.cat.id, artId: f.art.id }); } return; }
  ...
  if (ext) { (window as any).openExternal?.(ext.getAttribute('data-ext') || ''); return; }
  ...
  if (pdfBtn) {
    (window as any).openExternal?.(pdfUrl());
  ...
  if (dl) {
    ...
    if (art) { const f = findArticle(art[1]); if (f) { go({ view: 'art', part: f.cat.part, catId: f.cat.id, artId: f.art.id }); return; } }
    import('../core/deep_link_manager.js').then((m: any) => m.handleDeepLink?.(url)).catch(() => {});
    return;
  }
  ...
  if (anc) {
    ...
    (target as HTMLElement | undefined)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    return;
  }
  ...
  if (fold) {
    ...
    try { localStorage.setItem(`bmm_dh_fold_${which}`, String(now)); } catch { /* preference only */ }
    return;
  }
  ...
  if (dp) {
    ...
    if (route.view === 'page') void openPage(target, hash);
    else void showFullPage(target, null, hash);
    return;
  }
}
...
function anchorise(body: HTMLElement, hash?: string) {
  body.querySelectorAll('h3, h4, h5').forEach((h) => {
    if (!h.id) h.id = slugify(h.textContent || '');
  });
  if (!hash) return;
  const el = body.querySelector(`#${CSS.escape(hash)}`) as HTMLElement | null;
  if (el) setTimeout(() => el.scrollIntoView({ behavior: 'smooth', block: 'start' }), 60);
}

// ── every bundled page, browsable ────────────────────────────────────────────
...
async function loadManifest(): Promise<DocPageMeta[]> {
  ...
  try {
    ...
    _manifest = res.ok ? ((await res.json()).pages as DocPageMeta[]) : [];
  } catch { _manifest = []; }
  return _manifest;
}

const SECTION_TITLE: Record<string, L> = {
...
function asideFolded(which: string): boolean {
    try { return localStorage.getItem(`bmm_dh_fold_${which}`) === 'true'; } catch { return false; }
}
...
async function fillPage(path: string) {
  ...
  // A bundled page: ours, generated by sync-docs from the BMM Docs repository.
  body.innerHTML = renderDocMarkdown(stripTitle(md), { trusted: true });
  _filled = key;
  anchorise(body, hash);
...
async function showFullPage(rawPath: string, btn: HTMLElement | null, hash?: string, auto = false) {
  ...
  const setLabel = (b: HTMLElement | null, which: 'short' | 'full') => {
  ...
  if (!auto) body.innerHTML = `<p class="dh-lead">${tr({ en: 'Loading the full page…', fr: 'Chargement de la page complète…' })}</p>`;
  const md = await fetchDocPage(path);
  if (!md) {
    // The bundle is generated from a sibling repo, so it can legitimately be absent.
    body.innerHTML = article.dataset.shortHtml;
    if (auto) return;                      // nothing was promised, so say nothing
    try {
  ...
  if (!auto) article.scrollIntoView({ block: 'start', behavior: 'smooth' });
  anchorise(body, hash);
  await hydrateDocPage(body);
}
...
function fitDiagram(host: HTMLElement) {
...
export function initDocsHub() {
  ...
  (window as any).openDocsPageById = (artId: string) => {
    const f = findArticle(artId);
    if (!f) { (window as any).openDocsHome(); return; }
    const path = f.art.docsPath;
    if (path) {
      void loadManifest().then(() => {
        const known = (_manifest || []).some((p) => p.path === path);
        if (known) { route = { ...route, view: 'page', page: path, part: f.cat.part, catId: f.cat.id, artId: f.art.id }; showDocs(); paint(); }
        else openArticle(f.cat.id, f.art.id);
      });
    } else openArticle(f.cat.id, f.art.id);
  };
