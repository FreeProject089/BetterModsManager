...
function describe(method: string, path: string): { label: string; icon: string } {
    ...
    const map: Array<[RegExp, string, string]> = [
        ...
        [/^\/api\/mods\/all$/,           tl('actionListAllMods', 'List all mods (all profiles)'), ICN.list],
        [/^\/api\/mods$/,                tl('actionListMods', 'List mods'),         ICN.list],
        [/^\/api\/data\/export$/,        tl('actionDataExport', 'Export BMM data'), ICN.package],
        [/^\/api\/data\/import$/,        tl('actionDataImport', 'Import BMM data'), ICN.package],
        [/^\/api\/data$/,                tl('actionDataDump', 'Export data.json'),  ICN.package],
        [/^\/api\/modlists\/export$/,    tl('actionModlistExport', 'Export mod list'), ICN.package],
        [/^\/api\/modlists\/import$/,    tl('actionModlistImport', 'Import mod list'), ICN.package],
        [/^\/api\/modpacks\/export$/,    tl('actionModpackExport', 'Export modpack'), ICN.package],
        [/^\/api\/modpacks\/import$/,    tl('actionModpackImport', 'Import modpack'), ICN.package],
        [/^\/api\/plugins\/export$/,     tl('actionPluginExport', 'Export plugin'), ICN.package],
        [/^\/api\/plugins\/import$/,     tl('actionPluginImport', 'Import plugin'), ICN.package],
        [/^\/api\/profiles\/import\/ovgme$/, tl('actionImportOvgme', 'Import OvGME profile'), ICN.plus],
        [/^\/api\/profiles\/import\/omm$/,   tl('actionImportOmm', 'Import OMM profile'), ICN.plus],
        [/^\/api\/repo\/update$/,        tl('actionRepoUpdate', 'Update repo'),     ICN.refresh],
        [/^\/api\/apps\/install$/,       tl('actionAppInstall', 'Install app'),     ICN.plus],
        [/^\/api\/apps\/launch$/,        tl('actionAppLaunch', 'Launch app'),       ICN.play],
        [/^\/api\/apps\/permissions(\/[^/]+)?$/, tl('actionAppPerms', 'App permissions'), ICN.id],
        [/^\/api\/apps\/[^/]+$/,         tl('actionAppUninstall', 'Uninstall app'), ICN.trash],
        [/^\/api\/apps$/,                tl('actionListApps', 'List installed apps'), ICN.list],
        [/^\/api\/catalog\/new$/,        tl('actionCatalogNew', 'Create catalog'),  ICN.plus],
        [/^\/api\/catalog\/apps(\/[^/]+)?$/, tl('actionCatalogApp', 'Catalog app'), ICN.edit],
        [/^\/api\/catalog$/,             tl('actionGetCatalog', 'Get catalog'),     ICN.list],
        [/^\/api\/language\/import$/,    tl('actionLangImport', 'Import language'), ICN.plus],
        [/^\/api\/language\/template$/,  tl('actionLangTemplate', 'Language template'), ICN.list],
        ...
        switch (action) {
            ...
            case 'repo/fetch-ssh': {
                ...
                pullStoredTarget(String(params?.dir || ''))
                    .then((bytes: number) => toast(
                        t('repo.ssh.pulled').replace('{n}', '✓').replace('{size}', String(bytes)),
                        'success', 7000))
                    .catch((e: unknown) => toast(String((e as Error)?.message || e), 'error', 9000));
                break;
            }
