...
function ensurePurify() {
    ...
    DP.addHook('afterSanitizeAttributes', (node: any) => {
        ...
        if (node.tagName === 'IFRAME') {
            ...
            if (!/^https:\/\/(www\.)?youtube(-nocookie)?\.com\//i.test(src)) { node.parentNode && node.parentNode.removeChild(node); return; }
...
if (typeof document !== 'undefined') {
    document.addEventListener('click', (e) => {
        ...
        navigator.clipboard?.writeText((code as HTMLElement).innerText).then(() => {
            const prev = btn.innerHTML; btn.innerHTML = t('update.copied') || 'Copied!';
    ...
    document.addEventListener('click', (e) => {
        const el = (e.target as HTMLElement)?.closest?.('.bmm-replay-embed') as HTMLElement | null;
...
async function performUpdateCheck(showNoUpdateToast = false) {
    ...
    try {
        ...
        try {
            const bi: any = await invoke('check_update_via_installer');
            if (bi && !bi.error) {
                ...
                if (bi.notes) info.release_notes = bi.notes;
            }
...
function showUpdateAvailableModal(info) {
    ...
    if (incrementalBtn && info.manifest_url && !info.major_bump) {
...
async function legalHashes(): Promise<{ tos: string | null; privacy: string | null }> {
    try {
        const r: any = await invoke('legal_fingerprint');
...
function acceptedCurrent(key: string, hashKey: string, current: string | null): boolean {
    ...
    if (!current) return legacy || !!stored;   // cannot verify → do not nag
    if (stored) return stored === current;
    if (legacy) { localStorage.setItem(hashKey, current); return true; }
    return false;
}
...
export async function checkAutoPrivacy(): Promise<void> {
    try {
        ...
        if (acceptedCurrent(PRIVACY_SEEN_KEY, PRIVACY_HASH_KEY, privacy)) return;
        await openPrivacyModal();
        localStorage.setItem(PRIVACY_SEEN_KEY, 'true');
        if (privacy) localStorage.setItem(PRIVACY_HASH_KEY, privacy);
    } catch (e) {
...
export function markEulaAccepted() {
    ...
    legalHashes().then(({ tos }) => { if (tos) localStorage.setItem(EULA_HASH_KEY, tos); });
}
