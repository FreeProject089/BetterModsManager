...
function _loadSimple(): Promise<void> {
    if (!_simpleP) {
        _simpleP = fetch('assets/icons/simple-icons.json')
            .then(r => r.json()).then(j => { _simple = j; })
            .catch(() => { _simple = {}; });
    }
    return _simpleP;
}
...
export async function ensurePackFor(ref: string): Promise<void> {
    ...
    else if (ref.startsWith('si:')) await _loadSimple();
}
...
export function openIconPicker(opts: { current?: string } = {}): Promise<string | null> {
    return new Promise((resolve) => {
        ...
        void _loadLucide().then(() => (src === 'si' ? _loadSimple() : Promise.resolve())).then(() => { render(); search.focus(); });
    });
}
