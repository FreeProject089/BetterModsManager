        ...
        try {
            ...
            await listen('sha-status-changed', async (event: any) => {
                ...
                if (payload.status === 'calculating') {
                    ...
                    if (cidHint) cidHint.textContent = t('mod.hashingContent');
...
async function main() {
    ...
    try {
        ...
        if (ho?.applied) {
            ...
            for (const [k, v] of [
                ...
                if (typeof v === 'boolean') localStorage.setItem(k, String(v));
            }
    ...
    document.getElementById('btn-docs-resumable')?.addEventListener('click', () => ((window as any).openDocsPageById || openDiagram)('server-sync'));
