...
async function handleDeepLink(urlStr, originIn = 'unknown') {
    ...
    try {
        ...
        if (action === 'view/open') {
            ...
            if (!id)
                return;
            const item = document.querySelector(`.nav-item[data-view="${CSS.escape(id)}"]`);
            if (item)
                item.click();
            else
        ...
        if (action === 'catalog/import') {
            ...
            if (ix.looksLikeIndex(doc)) {
                ...
                const kinds = want ? [want] : ix.ROUTABLE;
                let added = 0;
                for (const k of kinds) {
                    try {
                        const r = await ix.importIndexForType(doc, k, url, undefined, cs.writeSources);
                        added += r.added || 0;
                    }
            ...
            const kind = want || ix.INDEX_TYPES
        ...
        if (action === 'schedule/run' || action === 'schedule/enable') {
            ...
            const steps = (task.steps || []).filter((s) => !s.disabled).length;
        ...
        if (action === 'hook') {
            ...
            const ok = await window.confirmCustom(t('sched.dl.hookTitle'), `<p style="font-size:13px;line-height:1.5;margin:10px 0 4px;">${t('sched.dl.hookDesc')}</p>
                 <div style="font-size:12px;font-family:var(--font-mono);background:rgba(0,0,0,0.3);padding:6px 10px;border-radius:6px;margin-top:8px;word-break:break-all;">${escHtml(name)}</div>`, 'accent', { yesLabel: t('common.yes'), noLabel: t('common.no') });
            if (!ok)
                return;
            try {
