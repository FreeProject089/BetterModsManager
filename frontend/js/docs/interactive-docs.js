...
function fixClusterLabels() {
    ...
    if (!viewport)
        return; // Wait for pan-zoom to init
...
let lastTooltipIcon = null;
let lastTooltipLiteral = false;
...
function showTooltipImpl(key, iconClass, isLiteral) {
    ...
    const taskyContainer = document.getElementById('tasky-bubble-docs');
    if (!bubble || !explanationEl || !taskyContainer)
        return;
    ...
    if (finalIcon === 'info')
        finalIcon = 'icon-info';
    if (finalIcon === 'warning' || finalIcon === 'alert')
        finalIcon = 'icon-warning';
    if (finalIcon === 'verify')
        finalIcon = 'icon-verify';
    if (finalIcon === 'search')
        finalIcon = 'icon-search';
    if (finalIcon === 'layers')
        finalIcon = 'icon-layers';
    if (finalIcon === 'history')
        finalIcon = 'icon-history';
    if (finalIcon === 'play')
        finalIcon = 'icon-play';
    if (finalIcon === 'refresh')
        finalIcon = 'icon-refresh';
    if (finalIcon === 'minimize')
        finalIcon = 'icon-minimize';
    if (finalIcon === 'maximize')
        finalIcon = 'icon-maximize';
    if (finalIcon === 'close')
        finalIcon = 'icon-close';
    if (finalIcon === 'folder')
        finalIcon = 'icon-folder';
    if (finalIcon === 'edit')
        finalIcon = 'icon-edit';
    if (finalIcon === 'trash' || finalIcon === 'delete')
        finalIcon = 'icon-trash';
    if (finalIcon === 'toggle')
        finalIcon = 'icon-toggle';
    if (finalIcon === 'alert')
        finalIcon = 'icon-alert';
    if (finalIcon === 'package')
...
export function openHelpTo(targetKey) {
    ...
    setTimeout(() => {
        ...
        if (targetEl) {
            ...
            const container = (accordion || targetEl);
...
function updateTaskyMascot(file) {
    ...
    if (currentMascotUrl === newUrl)
        return; // Only load if different
 ...
 */
function applyNodeHighlight(nodeId, retryCount = 0) {
    const container = document.getElementById('mermaid-diagram-container');
    if (!container)
        return;
    ...
    let svgNode = container.querySelector(`.node[id="${nodeId}"]`) ||
...
//# sourceMappingURL=interactive-docs.js.map