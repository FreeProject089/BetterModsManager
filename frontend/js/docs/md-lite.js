...
const ico = (d) => `<svg viewBox="0 0 16 16" width="11" height="11" fill="none" stroke="currentColor"`
 ...
 */
function docLinkish(a, inner, cls, style) {
    ...
    if (/^doc-page:/i.test(href))
        return `<button type="button" class="${cls} dh-xref"${style} data-docpage="${escRaw(href.slice(9))}">${inner}</button>`;
    if (/^doc:/i.test(href))
        return `<button type="button" class="${cls} dh-xref"${style} data-art="${escRaw(href.slice(4))}">${inner}</button>`;
    if (/^bmm:\/\//i.test(href))
        return `<button type="button" class="${cls}"${style} data-deeplink="${escRaw(href)}">${inner}</button>`;
    if (/^#/.test(href))
        return `<button type="button" class="${cls}"${style} data-anchor="${escRaw(href.slice(1))}">${inner}</button>`;
    if (/^(https?:|mailto:)/i.test(href))
        return `<button type="button" class="${cls}"${style} data-ext="${escRaw(href)}">${inner}</button>`;
...
function inline(s) {
    ...
    const keep = (h) => { maths.push(h); return `${maths.length - 1}`; };
    ...
    s = s.replace(/:(?:time|at)\[([^\]]+)\](?:\{([^}]*)\})?/g, (_m, raw, rawAttrs) => {
        ...
        if (!r.ok)
            return `<span class="doc-time">${esc(String(raw))}</span>`;
    ...
    s = s.replace(/!\[([^\]]*)\]\(([^)\s]+)(?:\s+"[^"]*")?\)/g, (_m, alt, src) => `<img class="dh-md-img" src="${escRaw(src)}" alt="${esc(alt)}" loading="lazy">`);
    s = s.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (_m, txt, url) => {
        // `[label](doc:article-id)` → an in-app cross-link that opens another docs article.
        if (/^doc:/i.test(url))
            return `<button type="button" class="dh-xref" data-art="${escRaw(url.slice(4))}">${txt}</button>`;
        ...
        if (/^doc-page:/i.test(url))
            return `<button type="button" class="dh-xref" data-docpage="${escRaw(url.slice(9))}">${txt}</button>`;
        ...
        if (/^https?:\/\//i.test(url))
            return `<button type="button" class="dh-link dh-link-ext" data-ext="${escRaw(url)}">${txt}</button>`;
        if (/^bmm:\/\//i.test(url))
            return `<button type="button" class="dh-link" data-deeplink="${escRaw(url)}">${txt}</button>`;
        if (/^#/.test(url))
            return `<button type="button" class="dh-link" data-anchor="${escRaw(url.slice(1))}">${txt}</button>`;
...
const isFence = (line) => /^:::+\s*$/.test(line.trim());
let tabSeq = 0;
...
function takeIndented(lines, start) {
    const body = [];
    let i = start;
    while (i < lines.length && !lines[i].trim())
        i++; // skip the blank after the marker
...
function renderBlocks(lines) {
    ...
    let i = 0;
    const flushPara = [];
    const flushP = () => { if (flushPara.length) {
        out.push(`<p>${inline(flushPara.join(' '))}</p>`);
        flushPara.length = 0;
    } };
    while (i < lines.length) {
        ...
        if (/^===\s+"/.test(t)) {
            ...
            const id = 'tb' + (++tabSeq);
            out.push(`<div class="dh-tabs" data-tabs>` +
        ...
        if (h) {
            flushP();
            out.push(`<h${h[1].length === 1 ? 3 : h[1].length + 1}>${inline(h[2])}</h${h[1].length === 1 ? 3 : h[1].length + 1}>`);
            i++;
            continue;
        }
        // blockquote
        ...
        if (anchorOnly) {
            flushP();
            out.push(`<a id="${anchorOnly[1]}" class="dh-anchor"></a>`);
            i++;
            continue;
        }
        ...
        if (/^[-*]\s+/.test(t) || /^\d+\.\s+/.test(t)) {
            ...
            const isItem = (l) => /^[-*]\s+/.test(l.trim()) || /^\d+\.\s+/.test(l.trim());
            const raw = [];
            while (i < lines.length && isItem(lines[i])) {
                ...
                while (i < lines.length) {
                    ...
                    if (!c.trim() || isItem(c))
                        break;
                    if (/^(#{1,6}\s|>|```|!!!|\?\?\?|===\s|:::|\||(\*{3,}|-{3,}|_{3,})$)/.test(c.trim()))
                        break;
...
function renderDirective(dir, body) {
    ...
    if (name === 'divider') {
        ...
        if (v)
            cls.push(`doc-variant-${v}`);
        const sp = String(attrs.space || '').toLowerCase();
        if (['none', 'xs', 'sm', 'md', 'lg', 'xl'].includes(sp))
            cls.push(`doc-space-${sp}`);
    ...
    if (name === 'columns' || name === 'row')
        return `<div class="doc-columns">${inner()}</div>`;
    if (name === 'column' || name === 'col')
        return `<div class="doc-column">${inner()}</div>`;
 ...
 */
function tableOfContents(src) {
    if (!/^\s*::toc\b/m.test(src))
        return src;
    const heads = [];
...
function slugForAnchor(s) {
    return s.toLowerCase().trim().normalize('NFD').replace(/[\u0300-\u036f]/g, '')
        .replace(/[^\w\s-]/g, '').replace(/\s+/g, '-');
}
export function renderDocMarkdown(src, opts = {}) {
    if (!src)
        return '';
