...
function articleDocsPaths() {
    const out = new Set();
    for (const c of CATEGORIES)
        for (const a of c.articles)
            if (a.docsPath)
                out.add(a.docsPath.replace(/\/+$/, ''));
    return out;
}
function unattachedPages(part) {
...
function wireSearch() {
    ...
    const bar = input.closest('.dh-searchbar');
...
export function initDocsHub() {
    ...
    void loadManifest().then(() => { if (route.view === 'art' || route.view === 'hub' || route.view === 'search')
        paint(); });
