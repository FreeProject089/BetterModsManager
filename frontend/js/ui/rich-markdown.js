...
function zoneNote(tz) {
    ...
    if (dir === 'none')
        return '';
    if (dir === 'same')
        return t('md.sched.same', { here, tz });
...
function docTime(raw, a) {
    ...
    if (!r.ok)
        return `<span class="doc-time">${escHtml(raw)}</span>`;
...
export function stepMarker(kind, n) {
    if (kind === 'a' || kind === 'alpha')
        return n <= 26 ? String.fromCharCode(64 + n) : String(n);
    if (kind === 'i' || kind === 'roman')
        return ROMAN[n - 1] || String(n);
    if (kind === 'dot' || kind === 'none' || kind === 'bullet')
        return '•';
    return String(n);
}
function parseDirAttrs(s) {
    const out = {};
    if (!s)
        return out;
    const re = /([\w-]+)=("[^"]*"|'[^']*'|[^\s}]+)/g;
...
function mdInline(s) {
    ...
    try {
        return (m && typeof m.parse === 'function') ? m.parse(s) : escHtml(s);
    }
    catch {
        return escHtml(s);
    }
...
export function iconImg(name) {
    ...
    if (ph)
        return `<span class="md-inline-icon md-inline-icon--mask" data-ph="${escAttr(ph)}"></span>`;
    ...
    if (simple) {
        ...
        if (!url)
            return `<span class="md-inline-icon md-inline-icon--mask" data-lucide="circle"></span>`;
...
function itemState(raw) {
    ...
    if (v === 'done' || v === 'shipped' || v === 'complete')
        return 'done';
    if (v === 'progress' || v === 'doing' || v === 'active' || v === 'wip')
        return 'progress';
    return 'planned';
}
/**
 ...
 */
function renderTracker(json, title) {
    let data;
    try {
    ...
    const catHtml = cats.map((c) => {
        ...
        const own = items.length
            ? Math.round(items.reduce((a, it) => a + (itemState(it?.status) === 'done' ? 100 : (Number(it?.percent) || 0)), 0) / items.length)
            : 0;
 ...
 */
function stagesToTrackerJson(inner) {
    const cats = [];
    let cur = null;
    let status = 'planned';
    let pct = 0;
    ...
    for (const line of inner) {
        ...
        if (li && cur)
            cur.items.push({ label: li[1].trim(), status, percent: pct });
    }
    const kept = cats.filter((c) => c.items.length);
    return kept.length ? JSON.stringify({ categories: kept }) : '';
}
export function expandDocBlocks(md, opts = {}, _top = true) {
    ...
    if (_top && /^\s*::toc\b/m.test(s)) {
        ...
        for (const line of s.split('\n')) {
            ...
            if (h)
                heads.push({ level: h[1].length, text: h[2].replace(/[*_`]/g, '').trim() });
        }
    ...
    s = s.replace(/<doc-comment\b([^>]*)>([\s\S]*?)<\/doc-comment>/gi, (_m, attrs, inner) => {
        const at = (n) => { const mm = String(attrs).match(new RegExp(`data-${n}=("|')([\\s\\S]*?)\\1`, 'i')); return mm ? mm[2] : ''; };
        const text = at('comment'), link = at('link'), img = at('img'), video = at('video');
        if (!text && !img && !link && !video)
            return inner;
    ...
    s = s.replace(/:link\[([^\]]+)\](?:\{([^}]*)\})?/g, (_m, txt, rawAttrs) => {
        ...
        if (!href)
            return `<span class="doc-link-c"${style}>${escHtml(txt)}</span>`;
        const ext = /^https?:\/\//i.test(href) ? ' target="_blank" rel="noreferrer"' : '';
    ...
    s = s.replace(/:(?:time|at)\[([^\]]+)\](?:\{([^}]*)\})?/g, (_m, raw, rawAttrs) => docTime(String(raw).trim(), parseDirAttrs(rawAttrs || '')));
    ...
    s = s.replace(/==([^=\n]+?)==/g, '<mark>$1</mark>');
    s = s.replace(/\[\[([^\]|#\n]*)(?:#([^\]|\n]+))?(?:\|([^\]\n]+))?\]\]/g, (_m, page, hash, text) => `<span class="community-ref" title="${escAttr(String(page || hash || '').trim())}">${escHtml(String(text || page || hash || '').trim())}</span>`);
    // `:audio[Title]{src=…}` → a player; `:img[Alt]{src= width= height= align= caption=}` → a picture.
    s = s.replace(/:audio\[([^\]]*)\](?:\{([^}]*)\})?/g, (_m, txt, rawAttrs) => {
        const a = parseDirAttrs(rawAttrs || '');
        const src = abs(a.src || a.href || '');
        if (!src)
            return escHtml(txt);
    ...
    s = s.replace(/^[ \t]*:(file|ref|card)\[([^\]]*)\](\{[^}]*\})?[ \t]*$/gm, (_m, n, label, at) => `:::${n}[${label}]${at || ''}\n:::`);
    // The 3.0 leaves written on their own line — `::spotify{src=…}`, `::youtube{…}`, `::audio{…}`,
    // `::include{…}` — become blocks, so the scanner below draws them.
    s = s.replace(/^[ \t]*::?(spotify|youtube|yt|audio|include|embed-md|openapi|swagger|live|image|img)(\[[^\]]*\])?(\{[^}]*\})?[ \t]*$/gm, (_m, n, label, at) => `:::${n}${label || ''}${at || ''}\n:::`);
    ...
    s = s.replace(/(^|[^:]):(file|ref|card)\[([^\]]+)\](?:\{([^}]*)\})?/g, (_m, before, n, txt, rawAttrs) => {
        ...
        if (!href)
            return `${before}<span class="${cls}">${ico}${escHtml(txt)}</span>`;
        const ext = /^https?:\/\//i.test(href) ? ' target="_blank" rel="noreferrer"' : '';
    ...
    while (i < lines.length) {
        ...
        // the same in the app. Classes are the app's `community-*`, styled in community.css.
            ...
            for (const side of ['before', 'after']) {
                if (attrs[side])
                    body = body.replace(new RegExp(`(<div class="community-compare-label" data-side="${side}">)<span data-default>[^<]*</span>`), `$1${escHtml(attrs[side])}`);
            }
            ...
            out.push('', `<div class="community-stats">${innerMd}</div>`, '');
        }
        ...
        // ── B.MD 3.0 blocks ──
            ...
            if (src)
                out.push('', `<figure class="community-audio">${ttl ? `<figcaption class="community-audio-title">${escHtml(ttl)}</figcaption>` : ''}<audio class="community-audio-player" controls preload="none" src="${escAttr(src)}"></audio></figure>`, '');
        }
            ...
            if (m)
                out.push('', `<div class="community-embed community-embed-video"><iframe class="community-embed-frame" src="https://www.youtube-nocookie.com/embed/${escAttr(m[1])}${start ? `?start=${start}` : ''}" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; fullscreen" allowfullscreen></iframe></div>`, '');
        }
            ...
            if (m)
                out.push('', `<div class="community-embed community-embed-spotify${attrs.compact != null ? ' community-embed-compact' : ''}"><iframe class="community-embed-frame" src="https://open.spotify.com/embed/${m[1]}/${escAttr(m[2])}" loading="lazy" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture"></iframe></div>`, '');
        }
            ...
            out.push('', `<div class="community-cards">${innerMd}</div>`, '');
        }
        else if (name === 'columns' || name === 'row') {
            out.push('', `<div class="community-columns">${innerMd}</div>`, '');
        }
        else if (name === 'column' || name === 'col') {
            out.push('', `<div class="community-column">${mdInline(innerMd)}</div>`, '');
        }
