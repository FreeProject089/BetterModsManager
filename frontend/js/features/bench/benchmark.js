...
export async function initBenchmark() {
    ...
    await listen('benchmark-point', (event) => {
        ...
        if (benchmarkPoints.length > MAX_DISPLAY_POINTS)
            benchmarkPoints.shift();
        ...
        if (currentEvent) {
            ...
            if (isFinished) {
                setTimeout(() => {
                    ...
                    if (lastPoint)
                        updateFloatingMonitor(lastPoint);
                }, 5000);
            }
        }
        updateBenchmarkUI();
    ...
    if (btnReset)
        btnReset.addEventListener('click', resetBenchmarkSession);
...
function updateFloatingMonitorState(active) {
    const pip = document.getElementById('benchmark-pip');
    if (active)
        pip.classList.add('is-tracking');
    else
        pip.classList.remove('is-tracking');
}
...
function syncPlaybackView() {
    if (playbackIndex < 0 || playbackIndex >= fullBenchmarkHistory.length)
        return;
    ...
    if (timeDisplay)
        timeDisplay.textContent = `${playbackIndex + 1} / ${fullBenchmarkHistory.length}`;
}
...
function updateBenchmarkUI(isPlayback = false) {
    ...
    if (diskTitleSpan) {
        if (!diskTitleSpan.dataset.origText)
            diskTitleSpan.dataset.origText = diskTitleSpan.textContent;
...
function updateFloatingMonitor(point, isPlayback = false) {
    ...
    if (cpuBar) {
        ...
        if (cpuVal)
            cpuVal.textContent = point.cpu_usage.toFixed(0) + '%';
    }
    ...
    if (ramBar) {
        ...
        if (ramVal)
            ramVal.textContent = point.ram_usage + 'M';
    }
    ...
    if (diskBar) {
        ...
        if (diskVal)
            diskVal.textContent = formatted.replace(' MB/s', 'M').replace(' KB/s', 'K');
    }
...
function renderChart(elementId, data, maxValue, color, unit, maxPoints, currentValue, avgValue = 0) {
    ...
    if (parent) {
        ...
        if (badge)
            badge.textContent = `${currentValue} ${unit.includes('/') ? '' : unit}`;
    }
    if (data.length < 2) {
