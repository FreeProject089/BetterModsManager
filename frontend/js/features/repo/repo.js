            ...
            ? `<button type="button" class="repo-cat-fold${folded ? ' is-folded' : ''}" data-cat-fold>
               ...
               <div class="repo-cat-srcs"${folded ? ' hidden' : ''}>${urls.map((u) => {
                ...
                <div class="repo-cat-src${off ? ' is-off' : ''}">
                    ...
                    ${from ? `<span class="repo-cat-via" title="${escAttr(from)}">${escHtml((t('repo.cat.via') || 'via') + ' ' + originLabel(from))}</span>` : ''}
 ...
 */
async function openCatalogBuilder(onScreen) {
    ...
    const pull = async (url, alsoFollow) => {
        if (!/^https?:\/\//i.test(url)) {
            msg = { kind: 'bad', text: t('repo.cat.badurl') || 'Enter an http(s) address.' };
            paint();
            return;
        }
        busy = true;
        msg = { kind: 'ok', text: t('repo.cat.b.pulling') || 'Fetching…' };
        paint();
        try {
    ...
    function wire() {
        ...
        ov.querySelectorAll('[data-pull]').forEach((b) => b.addEventListener('click', () => {
            // Already followed, so it is not followed again — only its contents are pulled.
            void pull(b.dataset.pull || '', false);
        }));
        ov.querySelectorAll('.repo-cat-b-row input[type=checkbox]').forEach((b) => b.addEventListener('change', () => {
...
export function initRepo() {
    ...
    mountSshAction(document.getElementById('repo-ssh-mount'), {
        ...
        source: {
            dir: () => document.getElementById('repo-export-path')?.value?.trim() || '',
        },
    });
    ...
    document.querySelectorAll('#view-repo .repo-method-pill').forEach(pill => {
        pill.addEventListener('click', () => {
            const m = pill.dataset.methodPill || 'generate';
            const group = pill.closest('.repo-host-group');
            if (!group)
                return;
        ...
        if (section === 'sync') {
            ...
            if (prefill) {
                ...
                if (prefill.serveDir && hostPathInput) {
                    ...
                    const reqCb = document.getElementById('repo-host-require-login');
                    if (reqCb) {
                        invoke('get_repo_require_login', { repoDir: prefill.serveDir }).then((v) => { reqCb.checked = !!v; }).catch(() => { });
                    }
                }
    ...
    const initRepoBrowser = () => {
        ...
        const fetchRepoList = async () => {
            ...
            try {
                ...
                const text = await invoke('fetch_remote_json', { url: bustUrl });
        ...
        const renderRepoList = () => {
            ...
            const isBoosted = (r) => r.featured === true || (r.featuredUntil && new Date(r.featuredUntil) > new Date());
    ...
    const initRepoUpdate = () => {
        ...
        const loadRepoFolder = async (folder) => {
            try {
        ...
        const refreshPublishBtn = async () => {
            const btn = document.getElementById('repo-update-ssh-action');
            if (!btn)
                return;
        ...
        const setMode = (remote) => {
            ...
            const block = document.getElementById('repo-update-remote-block');
            if (block)
                block.hidden = !remote;
            ...
            if (hint)
                hint.textContent = t(remote ? 'repo.update.folderHintRemote' : 'repo.update.folderHintLocal');
            if (pathInput)
        ...
        mountSshAction(document.getElementById('repo-update-ssh-action'), {
            ...
            source: {
                dir: () => document.getElementById('repo-update-path')?.value?.trim() || '',
            },
            ...
            onDone: (_n, dir) => {
                if (dir !== 'down')
                    return;
                const folder = document.getElementById('repo-update-path')?.value?.trim() || '';
                if (folder)
                    void loadRepoFolder(folder);
            },
        });
    ...
    //    a tutorial step or a deep link that targets either group always finds it on screen. ──
    {
        ...
        const apply = (v) => {
            document.querySelectorAll('.repo-host-group').forEach((g) => { g.style.display = (v === 'all' || g.dataset.hostGroup === v) ? '' : 'none'; });
            document.querySelectorAll('.repo-host-view-btn').forEach((b) => b.classList.toggle('active', b.dataset.hostView === v));
        };
        let cur = 'all';
        try {
            cur = localStorage.getItem(KEY) || 'all';
        }
        catch { /* private mode */ }
        if (document.getElementById('repo-host-view'))
            apply(cur);
        document.addEventListener('click', (e) => {
            const b = (e.target instanceof Element) ? e.target.closest('.repo-host-view-btn') : null;
            if (!b)
                return;
            const v = b.dataset.hostView || 'all';
            apply(v);
            try {
        ...
        const layerObs = new MutationObserver((muts) => {
            for (const m of muts)
                for (const node of m.addedNodes) {
                    if (!(node instanceof HTMLElement) || !node.classList.contains('tut-highlight'))
                        continue;
                    const target = node._tutTarget;
                    const grp = target?.closest('.repo-host-group');
                    if (grp && grp.style.display === 'none')
                        window.dispatchEvent(new Event('bmm:repo-host-reveal'));
                }
        });
    ...
    if (elements.btnStartExport) {
        elements.btnStartExport.addEventListener('click', async () => {
            ...
            try {
                ...
                if (creds === null) {
                    toast(t('mm.creds.noPass'), 'warning', 8000);
                    return;
                }
    ...
    // work?" button on step 1 brings it back, which is where somebody would look for it.
    {
        ...
        if (lede && localStorage.getItem(KEY) === '1')
            lede.hidden = true;
        document.getElementById('repo-host-lede-x')?.addEventListener('click', () => {
            ...
            catch { /* private mode: closed for this session only */ }
        });
