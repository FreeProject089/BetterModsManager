...
function setupFilters() {
    ...
    const applyHintState = (hidden) => {
        if (hintBanner)
            hintBanner.hidden = hidden;
        if (hintShow)
            hintShow.hidden = !hidden;
    };
    try {
        applyHintState(localStorage.getItem(HINT_KEY) === '1');
    }
    catch { /* ignore */ }
...
function updateSelectionCounter() {
    ...
    if (el)
        el.textContent = `${selectedPaths.size} ${t("modpack.modsSelected")}`;
}
