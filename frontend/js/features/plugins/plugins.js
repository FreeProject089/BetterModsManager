...
function buildPluginCard(plugin, source) {
    ...
    const act = (name, fn) => _plugActions.set(`${manifest.id}:${name}`, fn);
    ...
    act('au', () => {
        ...
        const row = card.querySelector('.plug-btn-au');
        row?.classList.toggle('plug-au-on', on);
        if (row)
            row.innerHTML = `${IC.refresh} ${escHtml(on
 ...
 */
const _plugActions = new Map();
let _plugDelegated = false;
function _wirePlugMenuDelegation() {
    if (_plugDelegated)
        return;
    _plugDelegated = true;
...
function renderScripts(container) {
    ...
    _scriptClickHandler = (e) => {
        ...
        if (dlOpenBtn) {
            ...
            if (url) {
                window.__bmmDeeplink?.(url, 'panel');
...
function wireCompareLinks(ov) {
    ...
    ov.querySelector('#plug-cmp-add-repo')?.addEventListener('click', async (ev) => {
        ...
        document.getElementById('nav-repo')?.click();
        setTimeout(() => {
            const box = document.getElementById('repo-sync-url');
            if (!box) {
                toast(url, 'info', 12000);
                return;
            }
            box.value = url;
