...
import { enabledOnly, isDisabled, setDisabled, originOf, originLabel, forgetOrigin, recordHistory, } from '../catalogs/catalog-index.js';
/**
...
async function handleSaveScriptZip() {
    ...
    const gitignore = `.env\n*.log\n__pycache__/\nnode_modules/\ntarget/\n`;
    // Build README.md
    const extMap = { bat: 'cmd', ps1: 'powershell', vbs: 'cscript', py: 'python', lua: 'lua', js: 'node', rb: 'ruby', php: 'php', go: 'go run', java: 'javac + java', cs: 'dotnet run', rs: 'cargo run' };
...
async function renderPerms(container) {
    ...
    container.querySelector('#plug-cors-any')?.addEventListener('change', async (e) => {
        ...
        if (spec)
            spec.style.cssText = on ? 'opacity:.45;pointer-events:none;' : '';
    });
...
// ── Action Handlers ────────────────────────────────────────────────────────
async function handleInstall(downloadUrl, name) {
    if (!downloadUrl) {
