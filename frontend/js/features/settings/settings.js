...
const _renderStorageModal = async () => {
    ...
    try {
        ...
        const gpuBox = document.getElementById('chk-webview-gpu');
        ...
        invoke('get_hardware_info').then((hw) => {
            ...
            const gpus = (hw.gpus || []);
            if (hw.gpu_timed_out)
                line(`GPU: ${t('storage.hwGpuSlow')}`);
            else if (!gpus.length)
                line(`GPU: ${t('storage.hwNoGpu')}`);
            for (const g of gpus)
                line(`GPU: ${g.name}${g.software ? ` (${t('storage.hwSoftware')})` : g.dedicated_mb ? ` · ${Math.round(g.dedicated_mb / 1024)} GB` : ''}`);
        ...
        gpuBox?.addEventListener('change', async (e) => {
            try {
...
async function initSecuritySettings() {
    ...
    function updateUI(mode) {
        ...
        if (ciFull)
            ciFull.style.display = full ? 'flex' : 'none';
        if (ciLim)
            ciLim.style.display = full ? 'none' : 'flex';
...
async function initSecurityInfoCard() {
    ...
    if (gotoCors && !gotoCors.dataset.bound) {
        ...
        gotoCors.addEventListener('click', () => {
            document.querySelector('.nav-item[data-view="plugins"]')?.click();
            ...
            let tries = 0;
            const go = () => {
                document.querySelector('.plug-tab[data-tab="perms"]:not(.active)')?.click();
                const el = document.getElementById('plug-cors-any')?.closest('.plug-section-card');
                if (el) {
    ...
    try {
        ...
        await kr.renderKeyManager('sic-keyring-list', 'sic-key-name', 'btn-sic-key-add', (k, kind) => toast(t(k), kind, kind === 'warning' ? 6000 : 3000), t);
        // ── Making one, here, where the ring is ───────────────────────────────
        document.getElementById('btn-sic-make-go')?.addEventListener('click', async () => {
            const kind = document.getElementById('sic-make-type')?.value || 'ed25519';
            const typed = document.getElementById('sic-make-name')?.value?.trim() || '';
            ...
            if (!name) {
                name = 'BMM';
                for (let n = 2; taken.has(name) && n < 999; n += 1)
                    name = `BMM ${n}`;
            }
            if (taken.has(name)) {
            ...
            try {
                ...
                const pub = document.getElementById('sic-make-pub');
                const where = document.getElementById('sic-make-where');
                if (pub)
                    pub.value = String(res?.public || '');
                ...
                if (where)
                    where.textContent = String(res?.path || '');
                if (out)
                    out.hidden = false;
                await kr.renderKeyManager('sic-keyring-list', 'sic-key-name', 'btn-sic-key-add', (k, kk) => toast(t(k), kk, kk === 'warning' ? 6000 : 3000), t);
        ...
        document.getElementById('btn-sic-copy-pub')?.addEventListener('click', async () => {
            ...
            try {
                ...
                toast(t('common.copyFailed'), 'warning');
            }
        });
    }
    catch (_) { }
...
async function initCatalogIndexSettings() {
    ...
    const makeFold = document.getElementById('cat-index-make-fold');
    if (makeFold) {
        const listEl = document.getElementById('cat-index-make-list');
        const paint = async () => {
            if (!listEl)
                return;
    ...
    void (async () => {
        const kr = await import('../../core/identity-key.js');
        const urlOf = () => document.getElementById('cat-index-key-url')?.value?.trim() || '';
