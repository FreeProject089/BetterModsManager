...
export async function initProfiles() {
    ...
    // redrew them.
    if (!window._profLangWired) {
        window._profLangWired = true;
...
async function pickProfileIconFile(prefix) {
    ...
    if (hidden)
        hidden.value = path; // source path to import on save
...
function clearProfileIconUI(prefix) {
    const hidden = document.getElementById(prefix + '-icon-image');
    if (hidden)
        hidden.value = '__REMOVE__'; // signal removal on save
    const prev = document.getElementById(prefix + '-custom-icon-preview');
    if (prev) {
...
async function confirmCreateProfile() {
    ...
    try {
        ...
        if (newIconSrc && newIconSrc !== '__REMOVE__') {
            try {
                await invoke('import_profile_icon', { profileId: profile.id, sourcePath: newIconSrc });
            }
            catch (e) {
        ...
        const isFirstProfile = (() => { try {
            return !localStorage.getItem('bmm_first_profile_done');
        }
        catch {
            return false;
        } })();
        if (isFirstProfile) {
            try {
                localStorage.setItem('bmm_first_profile_done', '1');
            }
            catch { }
...
function celebrateFirstProfile() {
    ...
    requestAnimationFrame(() => { o.style.opacity = '1'; const c = o.querySelector('.fpc-card'); if (c)
...
async function confirmEditProfile() {
    ...
    try {
        ...
        const editIconSrc = document.getElementById('edit-prof-icon-image')?.value || '';
        if (editIconSrc === '__REMOVE__') {
            try {
                await invoke('remove_profile_icon', { profileId });
            }
            catch (e) {
                console.warn('[profiles] remove icon failed', e);
            }
        }
        else if (editIconSrc) {
            try {
                await invoke('import_profile_icon', { profileId, sourcePath: editIconSrc });
            }
            catch (e) {
                console.warn('[profiles] import icon failed', e);
            }
...
export async function renderProfiles() {
    ...
    if (activeModsContainer) {
        ...
        grid.querySelectorAll('.profile-mod-chip').forEach((chip) => {
            const modId = chip.dataset.modId;
            if (!modId)
                return;
            const paint = () => {
                grid.querySelectorAll('.profile-mod-chip').forEach((c) => {
                    const on = selectedGlobalModIds.has(c.dataset.modId || '');
                    c.style.borderColor = on ? 'var(--accent)' : 'var(--border)';
                    c.style.color = on ? 'var(--accent)' : 'var(--text-secondary)';
                });
                modItems.forEach((mi) => {
                    mi.classList.toggle('selected', selectedGlobalModIds.has(mi.dataset.modId || ''));
                });
            };
            const pick = (e) => {
            ...
            chip.addEventListener('keydown', (e) => { if (e.key === 'Enter' || e.key === ' ')
                pick(e); });
