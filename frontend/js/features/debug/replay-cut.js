...
const sortByStart = (a, b) => a.start - b.start;
/**
 ...
 */
export function planCuts(events, ranges) {
    const plan = { cuts: [], refused: [], snapped: [] };
    ...
    for (const raw of ranges) {
        ...
        if (end <= start) {
            plan.refused.push({ range: raw, reason: 'empty' });
            continue;
        }
        if (start >= last || end <= first) {
            plan.refused.push({ range: raw, reason: 'out_of_range' });
            continue;
        }
        ...
        if (anchor !== end)
            plan.snapped.push({ asked: end, applied: anchor });
 ...
 */
export function shiftPastCuts(ts, cuts) {
    let removed = 0;
    for (const c of cuts) {
        if (c.end <= ts)
 ...
 */
export function applyCuts(events, cuts) {
    if (!cuts.length)
        return events.slice();
    return events
...
//# sourceMappingURL=replay-cut.js.map