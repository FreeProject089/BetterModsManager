...
class DebugUI {
    ...
    evalJSCommand(cmd) {
        ...
        const append = (html) => {
            ...
            if (out.children.length === 1 && out.textContent?.includes('No JS errors'))
                out.innerHTML = '';
        ...
        try {
            ...
            if (result instanceof Promise) {
                ...
                result.then((v) => append(`<span style="color:#34d399">←</span> ${esc(fmt(v))}`), (e) => append(`<span style="color:#f87171">✗ ${esc(e?.message || e)}</span>`));
            }
            else {
