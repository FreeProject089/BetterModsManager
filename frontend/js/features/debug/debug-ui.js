    ...
    async openSourceFile(path, row) {
        ...
        try {
            ...
            if (isImage) {
                ...
                const numberedLines = lines.map((line, i) => `<span class="src-line" id="src-ln-${i + 1}" style="display:flex;"><span class="src-gutter" style="min-width:${gutterW}px;padding-right:12px;text-align:right;color:var(--text-muted);opacity:0.4;user-select:none;flex-shrink:0;">${i + 1}</span><span class="src-line-content" style="flex:1;">${line || ' '}</span></span>`).join('\n');
                editorArea.innerHTML = `
                ...
                // Find-in-file
                const findInput = editorArea.querySelector('#src-find-input');
                const findCount = editorArea.querySelector('#src-find-count');
                findInput?.addEventListener('input', () => {
                    ...
                    editorArea.querySelectorAll('.src-line-content').forEach(cell => {
                        ...
                        if (text.toLowerCase().includes(q)) {
                            ...
                            cell.innerHTML = (cell.innerHTML || '').replace(new RegExp(q.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'gi'), m => `<mark class="src-hl" style="background:rgba(245,158,11,0.35);border-radius:2px;">${m}</mark>`);
                        }
                    });
