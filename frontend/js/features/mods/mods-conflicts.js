...
export async function openGlobalConflictModal(preselectModId = null) {
    ...
    try {
            ...
            if (!seeded) {
                ...
                if (survivors.length <= 4)
                    for (const sv of survivors)
                        openCards.add(sv.item.sourceModId);
                else if (survivors.length)
                    openCards.add(preselectModId && survivors.some(sv => sv.item.sourceModId === preselectModId)
        ...
        container.onclick = (ev) => {
            const head = ev.target?.closest?.('[data-cflt-toggle]');
            if (!head || !container.contains(head))
                return;
            const id = head.dataset.cfltToggle || '';
            const card = head.closest('.cflt-card');
            if (!card)
                return;
            ...
            if (fold)
                fold.textContent = container.querySelector('.cflt-card:not(.is-open)')
...
export async function openConflictTree(mod1Id, mod2Id) {
    {
        ...
        try {
            ...
            if (bothOn) {
                try {
                    const [ordered] = await invoke('mod_order_get', { profileId: null });
          ...
          ${dir ? `<span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:9.5px;color:var(--text-muted)">${escHtml(dir)}</span>` : ''}
            ...
            container.querySelector('#conf-flip')?.addEventListener('click', async () => {
                ...
                try {
                    ...
                    const moved = await invoke('mod_order_set', { profileId: null, order: next });
