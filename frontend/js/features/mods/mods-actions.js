...
export async function toggleAllMods(forcedEnable = null) {
    window.bmmTrack?.('feature', { name: 'mods.toggleAll' });
...
async function _loadAllProfileMods() {
    try {
        _allProfileMods = await invoke('get_mods_all_profiles');
    }
    catch {
        _allProfileMods = {};
    }
...
function _resolveDep(depRef) {
    ...
    const m = S.allMods.find((m) => m.id === depRef);
...
export async function setupDependencyInput(inputId, listId, suggestionsId, initialDeps = []) {
    ...
    const renderChips = () => {
        ...
        <button style="background:none;border:none;color:var(--danger);cursor:pointer;padding:0 0 0 4px;font-size:14px;line-height:1;">&times;</button>`;
            ...
            if (crossProfile)
                chip.title = `${t('mod.crossProfileDep') || 'Cross-profile dependency'}: ${profileName}`;
    ...
    const updateSuggestions = () => {
        ...
        const sameProfileMatches = S.allMods.filter((m) => (!val || m.name.toLowerCase().includes(val) || m.id.toLowerCase().includes(val)) &&
        ...
        const activeModIds = new Set((S.allMods || []).map((m) => m.id));
        const crossProfileMatches = [];
        for (const [profId, profData] of Object.entries(_allProfileMods)) {
            ...
            if (isCurrent || profId === activeProfileId)
                continue;
            for (const m of profData.mods) {
                const depRef = `${profId}::${m.id}`;
                if (selectedIds.includes(depRef))
                    continue;
                if (input._currentModId && m.id === input._currentModId)
                    continue;
        ...
        if (sameProfileMatches.length) {
            ...
            html += sameProfileMatches.slice(0, 8).map((m) => `
        ...
        suggs.querySelectorAll('.suggestion-item').forEach(item => {
            ...
            item.onclick = (e) => {
                e.stopPropagation();
                const depRef = item.dataset.depRef;
...
export async function scanModsFolder() {
    try {
        window.bmmTrack?.('feature', { name: 'mods.scan' });
