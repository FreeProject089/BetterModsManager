...
import { actAttrsStop } from '../../core/inline-actions.js';
...
export function createModCard(mod) {
    ...
    card.querySelector('.btn-copy-repo-mod-id')?.addEventListener('click', async (e) => {
        e.stopPropagation();
        window.closeGlobalDropdown(true);
        const rid = e.currentTarget.dataset.repoModId || mod.repo_mod_id || mod.id;
        try {
    ...
    card.querySelector('.btn-remove-mod').addEventListener('click', async (e) => {
        ...
        if (warningText)
            warningText.innerHTML = t('mod.deleteWarning', { name: `<strong style="color:var(--text-primary)">${escHtml(mod.name)}</strong>` });
